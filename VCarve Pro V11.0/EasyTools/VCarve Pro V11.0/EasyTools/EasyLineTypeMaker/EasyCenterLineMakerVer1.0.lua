...
-- Localized Variables --
  ...
  if selection.IsEmpty then
      MessageBox("Select one (1) Open Line to create a Center Line")
      return false
  end
  if selection.Count > 1 then
      MessageBox("Select one (1) Open Line to make a Center Line")
      return false
  end
  ...
  local pt3 = Polar2D(pt1,  CenterLine.Angle,  CenterLine.length * 0.08)
  local pt4 = Polar2D(pt2,  CenterLine.RAngle, CenterLine.length * 0.08)

  -- local layer = job.LayerManager:GetLayerWithName("CenterLine")
  line = Contour(0.0)
  line:AppendPoint(pt1a)
  line:LineTo(pt1)
  group:AddTail(line)
  ...
  -- layer:AddObject(CreateCadContour(line), true)
  line = Contour(0.0)
  line:AppendPoint(pt3)
  line:LineTo(pt4)
  group:AddTail(line)
  -- layer:AddObject(CreateCadContour(line), true)
  AddGroupToJob(job, group, "CenterLine")
  job:Refresh2DView()
  return true
end  -- function end
-- ===================================================]]