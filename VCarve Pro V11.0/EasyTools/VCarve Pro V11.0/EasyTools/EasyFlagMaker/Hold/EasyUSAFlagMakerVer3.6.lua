...
require "strict"
local Flag             = {}
Flag.Gpt1              = Point2D(0.0, 0.0)
Flag.HoistFlag         = 30.000
Flag.Version           = "(Ver: 3.60)"
Flag.StripeX           = 1.2500
Flag.HoistUnion        = 0.5385
Flag.FlyFlag           = 1.9000
Flag.FlyUnion          = 0.7600
Flag.UnionStarCentersV = 0.0540
Flag.UnionStarCentersH = 0.0630
Flag.StarDia           = 0.0616
Flag.Strip             = 0.0769
-- ===================================================
...
function Stripe(job, p1, p2, p3, p4, Layer)
  DisplayMessageBox("pt4 = " .. tostring(p4))
  local line = Contour(0.0)
...
function DrawStripes(job)
  local pt1 = Flag.Gpt5
  local pt2 = Point2D(0, 0)
  local pt3 = Point2D(0, 0)
  local pt4 = Point2D(0, 0)
  local lay = "Stripe-Red"
  for _ = 7, 1 , -1 do
    pt2 = Polar2D(pt1,   0.0, Flag.FlyFlag - Flag.FlyUnion)
    pt3 = Polar2D(pt2, 270.0, Flag.Stripe)
    pt4 = Polar2D(pt1, 270.0, Flag.Stripe)
  ...
  lay = "Stripe-White"
  for _ = 6, 1 , -1 do
    pt2 = Polar2D(pt1,   0.0, Flag.FlyFlag)
    pt3 = Polar2D(pt2, 270.0, Flag.Stripe)
    pt4 = Polar2D(pt1, 270.0, Flag.Stripe)
    Stripe(job, pt1, pt2, pt3, pt4, lay)
    pt1 = pt4
...
function main()
  ...
  while Loops do
    ...
    Flag.Inquiry = InquiryNumberBox("USA Flag Data", "Enter the Flag Strip Height", Flag.StripeX)
  ...
  if (Flag.Inquiry and (Flag.StripeX > 0)) then
    Flag.HoistFlag         = 13.00000 * Flag.StripeX
    Flag.HoistUnion        = 0.538500 * Flag.HoistFlag
    Flag.FlyFlag           = 1.900000 * Flag.HoistFlag
    Flag.FlyUnion          = 0.760000 * Flag.HoistFlag
    Flag.UnionStarCentersV = 0.054000 * Flag.HoistFlag
    Flag.UnionStarCentersH = 0.063000 * Flag.HoistFlag
    Flag.StarDia           = 0.061600 * Flag.HoistFlag
    Flag.StarOutRadius     = 0.500000 * Flag.StarDia
    Flag.StarInRadius      = 0.190983 * Flag.StarDia
    Flag.Stripe            = 0.076900 * Flag.HoistFlag
-- ======
    Flag.Gpt1 = Point2D(0.000, 0.000)
