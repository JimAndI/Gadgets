...
DialogWindow.ProjectSDK  = "http://www.jimandi.com/GadgetHelp/EasyMDFDoorMaker/EasyMDFDoorMaker.html"
-- ====================================================]]

  local function DebugMarkPoint(Note, Pt, Size, LayerName)
--[[-- ==MarkPoint==
...
function DoorStyleAVertical(pt5, pt6, pt8)
  pt5 = Polar2D(pt5,   0.0, Door.BitRad)
  pt5 = Polar2D(pt5,  90.0, Door.BitRad)
  pt6 = Polar2D(pt6, 180.0, Door.BitRad)
  pt6 = Polar2D(pt6,  90.0, Door.BitRad)
  pt8 = Polar2D(pt8,   0.0, Door.BitRad)
  pt8 = Polar2D(pt8, 270.0, Door.BitRad)
...
function DoorStyleAHorizontal(pt5, pt6, pt8)
  ...
  if (1.0 < fltCount1) then
    ...
    LogWriter( "Error - Door: " .. D2S(Door.Height) .. " X " .. D2S(Door.Width) .. " Failed to Draw the Horizontal Flutes")
  end -- if end
  return true
end -- function end
-- ====================================================]]
function DoorStyleB()                                  -- Draws a Cathedral Style Door
  ...
  local pt9 = Polar2D(pt8,   0.0, StyleB.B)
  local pt0 = Polar2D(pt7, 180.0, StyleB.B)
  ...
  local WidthTest = (Door.Width > (StyleB.A * 2.0) + (StyleB.D * 2.0))
  local HeightTest = (Door.Height > (StyleB.B + StyleB.C))
  local RadTest = rad > seg
  local LegTest = (pt0.x > pt9.x)
  if WidthTest and HeightTest  and LegTest then
    if (Door.Width > (StyleB.A * 2.0) + (StyleB.D * 2.0)) and RadTest then
      Door.Arch = true
      local line = Contour(0.0)
      ...
      Door.Arch = false
      DrawBox(pt5, pt6, pt7, pt8, Door.LayerIFrame)
      LogWriter( "Error - Door: " .. D2S(Door.Height) .. " X " .. D2S(Door.Width) .. " Failed to Draw the Arch in the panel")
    end
    if StyleB.Fluting == "Vertical" then
       DoorStyleBVertical(pt5, pt6, pt8, rad, seg, chor)
     --    DoorStyleBVertical(Doorpt1)
    elseif StyleB.Fluting == "Horizontal" then
      DoorStyleBHorizontal(pt4, pt5, pt6, pt8, rad, seg)
      -- DoorStyleBHorizontal(Doorpt1)
...
-- ====================================================]]
function DoorCenter(corner, style)
  Door.CTR = Polar2D(Polar2D(corner, 0.0, Door.Width * 0.5), 90.0, Door.Height * 0.5)
  Door.ptT = Polar2D(Door.CTR,   90.0, (Door.Height * 0.5) - (StyleG.B + Door.BitRad)) -- Top
  Door.ptB = Polar2D(Door.CTR,  270.0, (Door.Height * 0.5) - (StyleG.B + Door.BitRad)) -- Bottom
  Door.ptR = Polar2D(Door.CTR,    0.0, (Door.Width * 0.5)  - (StyleG.A + Door.BitRad)) -- Right
  Door.ptL = Polar2D(Door.CTR,  180.0, (Door.Width * 0.5)  - (StyleG.A + Door.BitRad)) -- Left
  ...
  DebugMarkPoint("Door.CTR",  Door.CTR, 4, "Debug")
  DebugMarkPoint("Door.ptT",  Door.ptT, 4, "Debug")
  DebugMarkPoint("Door.ptB",  Door.ptB, 4, "Debug")
  DebugMarkPoint("Door.ptR",  Door.ptR, 4, "Debug")
  DebugMarkPoint("Door.ptL",  Door.ptL, 4, "Debug")
  DebugMarkPoint("Door.ptTc", Door.ptTc, 4, "Debug")
  DebugMarkPoint("Door.ptBc", Door.ptBc, 4, "Debug")
  DebugMarkPoint("Door.ptRc", Door.ptRc, 4, "Debug")
  DebugMarkPoint("Door.ptLc", Door.ptLc, 4, "Debug")
  DebugMarkPoint("Door.CHR",  Door.CHR, 4, "Debug")
  DebugMarkPoint("Door.CHT",  Door.CHT, 4, "Debug")
  DebugMarkPoint("Door.CHL",  Door.CHL, 4, "Debug")
  DebugMarkPoint("Door.CHB",  Door.CHB, 4, "Debug")

  return true
end -- function end
-- ====================================================]]
function DoorStyleBHorizontalx(pt1)

  Door.CTR  = Polar2D(Polar2D(pt1, 0.0, Door.Width * 0.5), 90.0, Door.Height * 0.5) -- Door Center
  Door.ptT  = Polar2D(Door.CTR,   90.0, (Door.Height * 0.5) - (StyleB.C + Door.BitRad)) -- Top
  Door.ptB  = Polar2D(Door.CTR,  270.0, (Door.Height * 0.5) - (StyleB.C + Door.BitRad)) -- Bottom
  Door.ptR  = Polar2D(Door.CTR,    0.0, (Door.Width * 0.5)  - (StyleB.A + Door.BitRad)) -- Right
  Door.ptL  = Polar2D(Door.CTR,  180.0, (Door.Width * 0.5)  - (StyleB.A + Door.BitRad)) -- Left
  Door.ptTc = Polar2D(Door.ptT,  90.0, Door.BitRad) -- Top Rad Ctr
  Door.ptBc = Polar2D(Door.ptB, 270.0, Door.BitRad) -- Bottom Rad Ctr
  Door.ptRc = Polar2D(Door.ptR,   0.0, Door.BitRad) -- Right Rad Ctr
  Door.ptLc = Polar2D(Door.ptL, 180.0, Door.BitRad) -- Left Rad Ctr
  Door.CHR  = Polar2D(Door.CTR,    0.0, StyleB.FluteSpacing) -- Left
  Door.CHT  = Polar2D(Door.CTR,   90.0, StyleB.FluteSpacing) -- Top
  Door.CHL  = Polar2D(Door.CTR,  180.0, StyleB.FluteSpacing) -- Left
  Door.CHB  = Polar2D(Door.CTR,  270.0, StyleB.FluteSpacing) -- Bottom
...
--[[
  ...
  local ptAx = Polar2D(Door.CTR,   90.0, (Door.Height * 0.5) - StyleB.B)
  local ptA1 = Polar2D(ptAx,   0.0, (Door.Width * 0.5) - StyleB.D)
  local ptA2 = Polar2D(ptAx, 180.0, (Door.Width * 0.5) - StyleB.D)
  local chor = GetDistance(ptA1, ptA2)
  local seg  = GetDistance(ptA1, Door.ptTc)
  local rad  = ChordSeg2Radius(chor, seg)
  local ptA  = Door.ptR
  local ptB  = Door.ptL
  local ptC  = Door.ptR
  local ptD  = Door.ptL
  local Vdis = GetDistance(Door.ptT, Door.ptB) * 0.5
  local Hdis = GetDistance(Door.ptR, Door.ptL) * 0.5
  local fltCount1 = (GetDistance(ptAx, Door.ptB) / StyleB.FluteSpacing) - 2
  DrawLine(Door.ptL, Door.ptR, Door.LayerFlutes)

  -- DebugMarkPoint("ptAx",  ptAx, 4, "Debug")
  -- DebugMarkPoint("ptA1",  ptA1, 4, "Debug")
  -- DebugMarkPoint("ptA2",  ptA2, 4, "Debug")


  if (1.0 < fltCount1) then

    while fltCount1 > 1 do
      DrawLine(Polar2D(Door.CHT,  0.0, Hdis), Polar2D(Door.CHT, 180.0, Hdis), Door.LayerFlutes)
      DrawLine(Polar2D(Door.CHB,  0.0, Hdis), Polar2D(Door.CHB, 180.0, Hdis), Door.LayerFlutes)
      Door.CHT = Polar2D(Door.CHT,   90.0, StyleG.FluteSpacing) -- Move Up
      Door.CHB = Polar2D(Door.CHB,  270.0, StyleG.FluteSpacing) -- Move Down
      fltCount1 = fltCount1 - 2
      DebugMarkPoint("Door.CHT", Door.CHT, 5, "Debug")
    end -- while end

    Door.CHT = Polar2D(Door.CHT,  270.0, StyleG.FluteSpacing) -- Setback and move down

    -- local ptH = Polar2D(Door.CTR, 270.0, StyleB.C + Door.BitRad)
    local ptG = Polar2D(Door.CHT,  90.0, StyleB.FluteSpacing)
    -- local ptG = Door.CHT
    -- DebugMarkPoint("Door.ptT", Door.ptT, 5, "Debug")
    -- DebugMarkPoint("ptG", ptG, 4, "Debug")
    -- DebugMarkPoint("ptAx", ptAx, 4, "Debug")

    local fltArchCount = (GetDistance(Door.CHT, Door.ptT) / StyleB.FluteSpacing)
    seg = GetDistance(Door.CHT, Door.ptT)
    chord = RadSeg2Chord(rad, seg)

    while fltArchCount > 1 do
      ptC = Polar2D(Door.CHT,   0.0, chord * 0.5)
      ptD = Polar2D(Door.CHT, 180.0, chord * 0.5)
      DrawLine(ptC, ptD, Door.LayerFlutes)
...
-- ====================================================]]
function DoorStyleBHorizontal(pt4, pt5, pt6, pt8, rad, seg)
  pt5 = Polar2D(pt5,   0.0, Door.BitRad)
  pt5 = Polar2D(pt5,  90.0, Door.BitRad)
  pt6 = Polar2D(pt6, 180.0, Door.BitRad)
  pt6 = Polar2D(pt6,  90.0, Door.BitRad)
  ...
  pt8 = Polar2D(pt8, 270.0, Door.BitRad)
  rad = rad - Door.BitRad
  local chord =  RadSeg2Chord(rad, seg)
  local ptA = Polar2D(pt5,  90.0, (GetDistance(pt5, pt8) * 0.5))
  local ptB = Polar2D(pt6,  90.0, (GetDistance(pt5, pt8) * 0.5))
  local ptC = ptA
  local ptD = ptB
  ...
  if (1.0 < fltCount1) then
    while fltCount1 > 1 do
      ...
      DrawLine(ptA, ptB, Door.LayerFlutes)
      fltCount1 = fltCount1 - 2
    --  DebugMarkPoint("ptA",  ptA, 4, "Debug")
    end -- while end
    local ptQ = ptA
    while fltCount2 > 1 do
    ...
    local ptH = Polar2D(ptF, 270.0, StyleB.C + Door.BitRad)
    local fltArchCount = (GetDistance(ptG, ptH) / StyleB.FluteSpacing)
    seg = GetDistance(ptG, ptH)
    chord = RadSeg2Chord(rad, seg)
...
function DoorStyleC()                                  -- Draws a Double Cathedral Style Door
  ...
  local WidthTest = (Door.Width > (StyleC.A * 2.0) + (StyleC.D * 2.0))
  local HeightTest = (Door.Height > (StyleC.B + StyleC.C))
  local RadTest = rad > seg
  local LegTest = (pt12.x < pt11.x)


  if WidthTest and HeightTest and LegTest then
    if (Door.Width > (StyleC.A * 2.0) + (StyleC.D * 2.0)) and RadTest then
      local line = Contour(0.0)
      local layer = Door.job.LayerManager:GetLayerWithName(Door.LayerIFrame)
      line:AppendPoint(pt5)
      line:LineTo(pt6)
      ...
      LogWriter( "Error - Door: " .. D2S(Door.Height) .. " X " .. D2S(Door.Width) .. " Failed to Draw Arch for the inter panel")
    end
    if StyleC.Fluting == "Vertical" then
      DoorStyleCVertical(pt5, pt8, pt12, rad, seg, chor)
    elseif StyleC.Fluting == "Horizontal" then
      DoorStyleCHorizontal(Doorpt1, pt4, pt5, pt8, pt12, rad, seg)
...
function DoorStyleCVertical(pt5, pt8, pt12, rad, seg, chor)
  pt5 = Polar2D(pt5,   0.0, Door.BitRad)
  pt5 = Polar2D(pt5,  90.0, Door.BitRad)
  pt8 = Polar2D(pt8, 180.0, Door.BitRad)
  pt8 = Polar2D(pt8,  90.0, Door.BitRad)
  pt12 = Polar2D(pt12,   0.0, Door.BitRad)
  pt12 = Polar2D(pt12, 270.0, Door.BitRad)
  rad = rad - Door.BitRad
  chor =  RadSeg2Chord(rad, seg)
  local ptX = Polar2D(pt8, 180.0, StyleC.D)
...
-- ====================================================]]
function DoorStyleCHorizontal(pt1, pt4, pt5, pt8, pt12, rad, seg)
  pt5 = Polar2D(pt5,   0.0, Door.BitRad)
  pt5 = Polar2D(pt5,  90.0, Door.BitRad)
  pt8 = Polar2D(pt8, 180.0, Door.BitRad)
  pt8 = Polar2D(pt8,  90.0, Door.BitRad)
  ...
  if (1.0 < fltCount1) then
      ...
      local ptH = Polar2D(ptF, 270.0, StyleC.C + Door.BitRad)
      ...
      local ptN = Polar2D(ptK,  90.0, StyleC.C + Door.BitRad)
    if Door.Arch then
      local fltArchCount1 = (GetDistance(ptG, ptH) / StyleC.FluteSpacing)
      local fltArchCount2 = fltArchCount1
      local seg1 = GetDistance(ptG, ptH)
      local seg2 = seg1
      local chord = RadSeg2Chord(rad, seg1)
      while fltArchCount1 > 0 do
        ...
        seg1 = GetDistance(ptG, ptH)
        chord = RadSeg2Chord(rad, seg1)
      end -- while end
      seg2 = GetDistance(ptM, ptN)
      chord = RadSeg2Chord(rad, seg2)
      while fltArchCount2 > 1 do
  ...
  if (Door.Width > (StyleE.A * 2.0)) and  (Door.Height > (StyleE.B + StyleE.C)) then
    if (Door.Width > (StyleE.A * 2.0)) and (dis >= Door.BitRad) then
        ...
        elseif StyleE.Fluting == "Horizontal" then
          DoorStyleEHorizontal(pt4, pt5, pt6, pt8, rad, seg)
...
-- ====================================================]]
function DoorStyleEHorizontal(pt4, pt5, pt6, pt8, rad, seg)
  pt5 = Polar2D(pt5,   0.0, Door.BitRad)
  pt5 = Polar2D(pt5,  90.0, Door.BitRad)
  pt6 = Polar2D(pt6, 180.0, Door.BitRad)
  pt6 = Polar2D(pt6,  90.0, Door.BitRad)
...
-- ====================================================]]
function DoorStyleF()                                 -- Draws a Double Arched Style Door
  local pt2 = Polar2D(Doorpt1,   0.0, Door.Width)
  local pt3 = Polar2D(pt2,      90.0, Door.Height)
  local pt4 = Polar2D(Doorpt1,  90.0, Door.Height)
  DrawBox(Doorpt1, pt2, pt3, pt4, Door.LayerOFrame)

  local CTR = Polar2D(Polar2D(Doorpt1, 0.0, Door.Width * 0.5),      90.0, Door.Height * 0.5)
  local ptTc = Polar2D(CTR,  90.0, Door.Height * 0.5)
  local ptBc = Polar2D(CTR, 270.0, Door.Height * 0.5)
  local ptRc = Polar2D(CTR,   0.0, Door.Width * 0.5)
  local ptLc = Polar2D(CTR, 180.0, Door.Width * 0.5)
--[[
    DebugMarkPoint("CTR",  CTR,  4, "Debug")
    DebugMarkPoint("ptTc", ptTc, 4, "Debug")
    DebugMarkPoint("ptBc", ptBc, 4, "Debug")
    DebugMarkPoint("ptRc", ptRc, 4, "Debug")
    DebugMarkPoint("ptLc", ptLc, 4, "Debug")
]]
  local ptTf = Polar2D(CTR,  90.0, (Door.Height * 0.5) - StyleF.C) -- Top inside face
  local ptBf = Polar2D(CTR, 270.0, (Door.Height * 0.5) - StyleF.C) -- Bottom inside face
  local ptRf = Polar2D(CTR, 180.0, (Door.Width  * 0.5) - StyleF.A) -- Right inside face
  local ptLf = Polar2D(CTR,   0.0, (Door.Width  * 0.5) - StyleF.A) -- Left inside face
--[[
    DebugMarkPoint("ptTf", ptTf, 4, "Debug")
    DebugMarkPoint("ptBf", ptBf, 4, "Debug")
    DebugMarkPoint("ptRf", ptRf, 4, "Debug")
    DebugMarkPoint("ptLf", ptLf, 4, "Debug")
]]
  local ptTp1 = Polar2D(ptLf,  90.0, (Door.Width * 0.5) - StyleF.B) -- Top Left
  local ptTp2 = Polar2D(ptRf,  90.0, (Door.Width * 0.5) - StyleF.B) -- Top Right
  local ptBp1 = Polar2D(ptLf, 270.0, (Door.Width * 0.5) - StyleF.B) -- Bottom Left
  local ptBp2 = Polar2D(ptRf, 270.0, (Door.Width * 0.5) - StyleF.B) -- Bottom Right
--[[
    DebugMarkPoint("ptTp1", ptTp1, 4, "Debug")
    DebugMarkPoint("ptTp2", ptTp2, 4, "Debug")
    DebugMarkPoint("ptBp1", ptBp1, 4, "Debug")
    DebugMarkPoint("ptBp2", ptBp2, 4, "Debug")
]]
  local chor = GetDistance(ptTp1, ptTp2)
  local seg = StyleF.B - StyleF.C
  local rad = ChordSeg2Radius (chor, seg)
  local bulge =  Radius2Bulge (ptTp1, ptTp2, ChordSeg2Radius (chor, seg))

  local RadiusCheck = (rad > (chor * 0.5))
  local SegCheck = (rad > seg)
  local WidthCheck  = (Door.Width  > (StyleF.A * 2.0))
  local HeightCheck = (Door.Height > (StyleF.C * 2.0))
  ...
  if RadiusCheck and  WidthCheck and HeightCheck and SegCheck then
    if (Door.Width > (StyleF.A * 2.0)) then
      ...
      elseif StyleF.Fluting == "Horizontal" then
        DoorStyleFHorizontal(Doorpt1, pt4, pt5, pt6, pt8, rad, seg)
        -- DoorStyleFHorizontal(Doorpt1)
...
function DoorStyleFVertical(pt5, pt6, pt8, rad, seg, chor)
  ...
  local ptC = ptA
  local ptD = ptB
  local fltCount1 = (GetDistance(pt5, pt6) / StyleF.FluteSpacing) - 1
  local fltCount2 = fltCount1
...
-- ====================================================]]
function DoorStyleFHorizontalx(pt1)
  ...
  local ptT = Polar2D(CTR,   90.0, (Door.Height * 0.5) - (StyleG.C + Door.BitRad)) -- Top
  local ptB = Polar2D(CTR,  270.0, (Door.Height * 0.5) - (StyleG.C + Door.BitRad)) -- Bottom
  ...
  local ptRl = Polar2D(ptR,  90.0, (GetDistance(ptT, ptB) * 0.5) - StyleF.C)
  local ptRr = Polar2D(ptR, 270.0, (GetDistance(ptT, ptB) * 0.5) - StyleF.C)
  local ptLl = Polar2D(ptL,  90.0, (GetDistance(ptT, ptB) * 0.5) - StyleF.A)
  local ptLr = Polar2D(ptL, 270.0, (GetDistance(ptT, ptB) * 0.5) - StyleF.A)
  local CHR = Polar2D(CTR,    0.0, StyleF.FluteSpacing) -- Right
  local CHL = Polar2D(CTR,  180.0, StyleF.FluteSpacing) -- Left
  local CHT = Polar2D(CTR,   90.0, StyleF.FluteSpacing) -- Top
  local CHB = Polar2D(CTR,  270.0, StyleF.FluteSpacing) -- Bottom


DebugMarkPoint("ptLl", ptLl, 4, "Debug")
DebugMarkPoint("ptLr", ptLr, 4, "Debug")
DebugMarkPoint("ptX", ptX, 4, "Debug")
DebugMarkPoint("CTR", CTR, 4, "Debug")
DebugMarkPoint("ptT", ptT, 4, "Debug")
DebugMarkPoint("ptB", ptB, 4, "Debug")
DebugMarkPoint("ptR", ptR, 4, "Debug")
DebugMarkPoint("ptL", ptL, 4, "Debug")
DebugMarkPoint("ptTc", ptTc, 4, "Debug")
DebugMarkPoint("ptBc", ptBc, 4, "Debug")
DebugMarkPoint("ptRc", ptRc, 4, "Debug")
DebugMarkPoint("ptLc", ptLc, 4, "Debug")
DebugMarkPoint("CHR", CHR, 6, "Debug")
DebugMarkPoint("CHL", CHL, 6, "Debug")
DebugMarkPoint("CHT", CHT, 6, "Debug")
DebugMarkPoint("CHB", CHB, 6, "Debug")

  local Vdis = GetDistance(ptT, ptB) * 0.5
  local Hdis = GetDistance(ptR, ptL) * 0.5
  local fltCount = (GetDistance(ptRl, ptRr) / StyleF.FluteSpacing) - 1

  DrawLine(ptR, ptL, Door.LayerFlutes)

  CHT = Polar2D(CHT,  270.0, StyleF.FluteSpacing) -- Top SetBack
  CHB = Polar2D(CHB,   90.0, StyleF.FluteSpacing) -- Bottom SetBack

  if (1.0 < fltCount) then
    while fltCount > 1 do
      CHT = Polar2D(CHT,   90.0, StyleF.FluteSpacing) -- Move Up
      CHB = Polar2D(CHB,  270.0, StyleF.FluteSpacing) -- Move Down
    ...
    local Adis  = GetDistance(CHT, ptT)
           CHT  = Polar2D(CHT,  90.0, StyleF.FluteSpacing) -- Next Step
           CHB  = Polar2D(CHB, 270.0, StyleF.FluteSpacing) -- Next Step
    local seg   = GetDistance(CHB, ptB)
    local chord = RadSeg2Chord(Rad, seg)
...
-- ====================================================]]
function DoorStyleFHorizontal(pt1, pt4, pt5, pt6, pt8, rad, seg)
  local ptA1 = Polar2D(pt1,     0.0, Door.Width * 0.5)
  local ptA2 = Polar2D(ptA1,   90.0, StyleF.C)
  local ptA3 = Polar2D(ptA2,   90.0, rad)
         seg = RadChord2Segment(rad - Door.BitRad, GetDistance(pt5, pt6) - (Door.BitRad * 2.0 ))
  local chor =  RadSeg2Chord(rad, seg)
  local ptA4 = Polar2D(ptA3,  270.0, rad - Door.BitRad)
  local ptA5 = Polar2D(ptA4,   90.0, seg)
  local ptA6 = Polar2D(pt4,     0.0, Door.Width * 0.5)
  local ptA7 = Polar2D(ptA6,  270.0, GetDistance(ptA1, ptA5))
  pt6 = Polar2D(ptA5,   0.0, chor * 0.5)
  pt5 = Polar2D(ptA5, 180.0, chor * 0.5)
  pt8 = Polar2D(ptA7, 180.0, chor * 0.5)
  rad = rad - Door.BitRad
  chor =  RadSeg2Chord(rad, seg)
  local ptA = Polar2D(pt5,  90.0, (GetDistance(pt5, pt8) * 0.5))
  local ptB = Polar2D(pt6,  90.0, (GetDistance(pt5, pt8) * 0.5))
  ...
  if (1.0 < fltCount1) then
    ...
    local ptH = Polar2D(ptF, 270.0, StyleF.C + Door.BitRad)
    local ptJ = Polar2D(ptC,   0.0, (GetDistance(pt5, pt6) * 0.5))
    local ptK = Polar2D(pt1,   0.0, Door.Width * 0.5)
    local ptM = Polar2D(ptJ, 270.0, StyleF.FluteSpacing)
    local ptN = Polar2D(ptK,  90.0, StyleF.C + Door.BitRad)
    local fltArchCount1 = (GetDistance(ptE, ptH) / StyleF.FluteSpacing) -1
    local fltArchCount2 = fltArchCount1
    local seg1 = GetDistance(ptG, ptH)
    local seg2 = seg1
    local chord = RadSeg2Chord(rad, seg1)
...
--[[
DebugMarkPoint("ptBl", ptBl, 4, "Debug")
DebugMarkPoint("ptBr", ptBr, 4, "Debug")
DebugMarkPoint("ptX", ptX, 4, "Debug")
DebugMarkPoint("CTR", CTR, 4, "Debug")
DebugMarkPoint("ptT", ptT, 4, "Debug")
DebugMarkPoint("ptB", ptB, 4, "Debug")
...
DebugMarkPoint("ptRc", ptRc, 4, "Debug")
DebugMarkPoint("ptLc", ptLc, 4, "Debug")
DebugMarkPoint("CHR", CHR, 4, "Debug")
DebugMarkPoint("CHL", CHL, 4, "Debug")
]]
...
--   DebugMarkPoint("+CHR+", CHR, 3, "Debug")
...
function DoorStyleGHorizontal(pt1)
  ...
  local ptLr = Polar2D(ptL, 180.0, ((GetDistance(ptT, ptB) * 0.5) - StyleG.C) + Adj)
--[[
DebugMarkPoint("ptRl", ptBl, 4, "Debug")
DebugMarkPoint("ptRr", ptBr, 4, "Debug")
DebugMarkPoint("ptTl", ptBl, 4, "Debug")
DebugMarkPoint("ptTr", ptBr, 4, "Debug")
DebugMarkPoint("ptLl", ptBl, 4, "Debug")
DebugMarkPoint("ptLr", ptBr, 4, "Debug")
DebugMarkPoint("ptBl", ptBl, 4, "Debug")
DebugMarkPoint("ptBr", ptBr, 4, "Debug")
DebugMarkPoint("ptX", ptX, 4, "Debug")
DebugMarkPoint("CTR", CTR, 4, "Debug")
...
function OnLuaButton_InquiryMakeDoor(dialog)
  local runit = true
-- Door.Fluting = dialog:GetDropDownListValue("DoorFluting")
  Door.Style = dialog:GetDropDownListValue("DoorStyle")
