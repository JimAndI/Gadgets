...
function DoorStyleA()                                 -- Draws a Door Style A
  ...
  local pt5 = Polar2D(Polar2D(Doorpt1,   0.0, StyleA.A), 90.0, StyleA.B)
  local pt6 = Polar2D(pt5,   0.0, (Door.Width - (StyleA.A * 2.0)))
  local pt7 = Polar2D(pt6,  90.0, (Door.Height - (StyleA.B * 2.0)))
  local pt8 = Polar2D(pt5,  90.0, (Door.Height - (StyleA.B * 2.0)))
  local ptA = Polar2D(Doorpt1, 0.0, (Door.Width * 0.5))
  local ptB = Polar2D(pt4,     0.0, (Door.Width * 0.5))
  local ptC = Polar2D(ptA,    90.0, StyleA.B)
  local ptD = Polar2D(ptB,   270.0, StyleA.B)
  if (Door.Width > StyleA.A) and  (Door.Height > (StyleA.B * 2.0)) then
    if (Door.Width > (StyleA.A * 2.0)) and  (Door.Height > (StyleA.B * 2.0)) then
...
function DoorStyleAVertical(pt5, pt6, pt8)
  ...
  local ptC = ptA
  local ptD = ptB
  local fltCount1 = (GetDistance(pt5, pt6) / StyleA.FluteSpacing) - 1
  local fltCount2 = fltCount1
  DrawLine(ptA, ptB, Door.LayerFlutes)
  if (1.0 < fltCount1) then
    while fltCount1 > 1 do
      ptA = Polar2D(ptA, 0.0, StyleA.FluteSpacing)
      ptB = Polar2D(ptB, 0.0, StyleA.FluteSpacing)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      fltCount1 = fltCount1 - 2
    end -- while end
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 180.0, StyleA.FluteSpacing)
      ptD = Polar2D(ptD, 180.0, StyleA.FluteSpacing)
...
function DoorStyleAHorizontal(pt5, pt6, pt8)
  ...
  local ptC = ptA
  local ptD = ptB
  local fltCount1 = (GetDistance(pt5, pt8) / StyleA.FluteSpacing) - 1
  local fltCount2 = fltCount1
  DrawLine(ptA, ptB, Door.LayerFlutes)
  if (1.0 < fltCount1) then
    while fltCount1 > 1 do
      ptA = Polar2D(ptA, 90.0, StyleA.FluteSpacing)
      ptB = Polar2D(ptB, 90.0, StyleA.FluteSpacing)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      fltCount1 = fltCount1 - 2
    end -- while end
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 270.0, StyleA.FluteSpacing)
      ptD = Polar2D(ptD, 270.0, StyleA.FluteSpacing)
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltCount2 = fltCount2 - 2
    end -- while end
  end -- if end
...
function DoorStyleB()                                 -- Draws a Door Style B
  ...
  local pt9 = Polar2D(pt8,   0.0, StyleB.D)
  local pt0 = Polar2D(pt7, 180.0, StyleB.C)

  local chor = GetDistance(pt9, pt0)
  local seg = StyleB.B - StyleB.C
  ...
  if (Door.Width > (StyleB.A * 2.0)) and  (Door.Height > (StyleB.B + StyleB.C)) then
    if (Door.Width > (StyleB.A * 2.0) + (StyleB.D * 2.0)) and RadGood  then
      local line = Contour(0.0)
      ...
      elseif StyleB.Fluting == "Horizontal" then
        DoorStyleBHorizontal(pt4, pt5, pt6, pt8, rad)
...
function DoorStyleBVertical(pt5, pt6, pt8, rad, seg, chor)
  ...
  local ptC = ptA
  local ptD = ptB
  local fltCount1 = (GetDistance(pt5, pt6) / StyleB.FluteSpacing) - 1
  local fltCount2 = fltCount1
  ...
  if (1.0 < fltCount1) then
    ...
    while fltCount1 > 1 do
      ptA = Polar2D(ptA, 0.0, StyleB.FluteSpacing)
      ptB = Polar2D(ptB, 0.0, StyleB.FluteSpacing)
    ...
    chord = StyleB.FluteSpacing * 2.0
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 180.0, StyleB.FluteSpacing)
      ptD = Polar2D(ptD, 180.0, StyleB.FluteSpacing)
...
-- ====================================================]]
function DoorStyleBHorizontal(pt4, pt5, pt6, pt8, rad)
  local ptA = Polar2D(pt5,  90.0, (GetDistance(pt5, pt8) * 0.5))
  local ptB = Polar2D(pt6,  90.0, (GetDistance(pt5, pt8) * 0.5))
  local ptC = ptA
  local ptD = ptB
  local chor
  local sig
  local fltCount1 = (GetDistance(pt5, pt8) / StyleB.FluteSpacing) - 1
  local fltCount2 = fltCount1
  DrawLine(ptA, ptB, Door.LayerFlutes)
  if (1.0 < fltCount1) then
    while fltCount1 > 1 do
      ptA = Polar2D(ptA, 90.0, StyleB.FluteSpacing)
      ptB = Polar2D(ptB, 90.0, StyleB.FluteSpacing)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      fltCount1 = fltCount1 - 2
    end -- while end
    local ptQ = ptA
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 270.0, StyleB.FluteSpacing)
      ptD = Polar2D(ptD, 270.0, StyleB.FluteSpacing)
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltCount2 = fltCount2 - 2
    end -- while end
    local ptE = Polar2D(ptQ,   0.0, (GetDistance(pt5, pt6) * 0.5))
    local ptF = Polar2D(pt4,   0.0, Door.Width * 0.5)
    local ptG = Polar2D(ptE,  90.0, StyleB.FluteSpacing)
    local ptH = Polar2D(ptF, 270.0, StyleB.C)

    local fltArchCount = (GetDistance(ptG, ptH) / StyleB.FluteSpacing)
    sig = GetDistance(ptG, ptH)
    chord = RadSeg2Chord(rad, sig)
    while fltArchCount > 0 do
      ptC = Polar2D(ptG,   0.0, chord * 0.5)
      ptD = Polar2D(ptG, 180.0, chord * 0.5)
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltArchCount = fltArchCount - 1.0
      ptG = Polar2D(ptG, 90.0, StyleB.FluteSpacing)
      sig = GetDistance(ptG, ptH)
      chord = RadSeg2Chord(rad, sig)
    end -- while end
  end -- if end
...
function DoorStyleC()                                 -- Draws a Door Style D
  ...
  local pt4 = Polar2D(Doorpt1,  90.0, Door.Height)
  DrawBox(Doorpt1, pt2, pt3, pt4, Door.LayerOFrame)
  local pt5 = Polar2D(Polar2D(Doorpt1,   0.0, StyleC.A), 90.0, StyleC.B)
  local pt6 = Polar2D(pt5,   0.0, StyleC.D )
  local pt8 = Polar2D(pt5,   0.0, (Door.Width - (StyleC.A * 2.0)))
  local pt7 = Polar2D(pt8, 180.0, StyleC.D)
  local pt9 = Polar2D(pt8,   90.0, (Door.Height - (StyleC.B + StyleC.B)))
  local pt12 = Polar2D(pt5,  90.0, (Door.Height - (StyleC.B + StyleC.B)))
  local pt10 = Polar2D(pt9, 180.0, StyleC.D)
  local pt11 = Polar2D(pt12,   0.0, StyleC.D)
  local chor = GetDistance(pt6, pt7) -- Top Arc
  local seg = StyleC.B - StyleC.C
  local rad = ChordSeg2Radius (chor, seg)
  local bulge =  Radius2Bulge (pt6, pt7, ChordSeg2Radius (chor, seg))
  if (Door.Width > (StyleC.A * 2.0)) and  (Door.Height > (StyleC.B + StyleC.C)) then
    if (Door.Width > (StyleC.A * 2.0) + (StyleC.D * 2.0)) then
      local line = Contour(0.0)
      local layer = Door.job.LayerManager:GetLayerWithName(Door.LayerIFrame)
      line:AppendPoint(pt5)
      line:LineTo(pt6)
      ...
      if StyleC.Fluting == "Vertical" then
        DoorStyleCVertical(pt5, pt8, pt12, rad, seg, chor)
      elseif StyleC.Fluting == "Horizontal" then
        DoorStyleCHorizontal(Doorpt1, pt4, pt5, pt8, pt12, rad)
...
-- ====================================================]]
function DoorStyleCVertical(pt5, pt8, pt12, rad, seg, chor)
  local ptA = Polar2D(pt5,    0.0, (GetDistance(pt5, pt8) * 0.5))
  local ptB = Polar2D(pt12,   0.0, (GetDistance(pt5, pt8) * 0.5))
  local ptBa = Polar2D(ptB,  90.0, seg)
  local ptBb = Polar2D(ptA, 270.0, seg)
  local ptC = ptA
  local ptD = ptB

  local fltCount1 = (GetDistance(pt5, pt8) / StyleC.FluteSpacing) - 1
  local fltCount2 = fltCount1
  local chord = StyleC.FluteSpacing * 2.0
  DrawLine(ptBa, ptBb, Door.LayerFlutes)
  local Segment = (rad - (0.5 * (math.sqrt((4.0 * rad^2) - chord^2))))
  local Segment2 = Segment
  if (1.0 < fltCount1) then
    local Arcstep = seg - Segment
    while fltCount1 > 1 do
      ptA = Polar2D(ptA, 0.0, StyleC.FluteSpacing)
      ptB = Polar2D(ptB, 0.0, StyleC.FluteSpacing)
      ptBa = Polar2D(ptB, 90.0, Arcstep)
      ptBb = Polar2D(ptA, 270.0, Arcstep)
      if chor > chord then
        DrawLine(ptBb, ptBa, Door.LayerFlutes)
      else
        DrawLine(ptA, ptB, Door.LayerFlutes)
      end
      fltCount1 = fltCount1 - 2
      chord = chord + (StyleC.FluteSpacing * 2.0)
      Segment = (rad - (0.5 * (math.sqrt((4.0 * rad^2) - chord^2))))
      Arcstep = seg - Segment
    end -- while end
    Arcstep = seg - Segment2
    chord = StyleC.FluteSpacing * 2.0
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 180.0, StyleC.FluteSpacing)
      ptD = Polar2D(ptD, 180.0, StyleC.FluteSpacing)
      ptBa = Polar2D(ptD, 90.0, Arcstep)
      ptBb = Polar2D(ptC,270.0, Arcstep)
      if chor > chord then
        DrawLine(ptBa, ptBb, Door.LayerFlutes)
      else
        DrawLine(ptC, ptD, Door.LayerFlutes)
...
-- ====================================================]]
function DoorStyleCHorizontal(pt1, pt4, pt5, pt8, pt12, rad)
  local ptA = Polar2D(pt5,  90.0, (GetDistance(pt5, pt12) * 0.5))
  local ptB = Polar2D(pt8,  90.0, (GetDistance(pt5, pt12) * 0.5))
  local ptC = ptA
  local ptD = ptB
  local fltCount1 = (GetDistance(pt5, pt12) / StyleC.FluteSpacing) - 1
  local fltCount2 = fltCount1
  DrawLine(ptA, ptB, Door.LayerFlutes)  -- Center Line
  if (1.0 < fltCount1) then
    while fltCount1 > 1 do
      ptA = Polar2D(ptA, 90.0, StyleC.FluteSpacing)
      ptB = Polar2D(ptB, 90.0, StyleC.FluteSpacing)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      fltCount1 = fltCount1 - 2
    end -- while end
    local ptQ = ptA
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 270.0, StyleC.FluteSpacing)
      ptD = Polar2D(ptD, 270.0, StyleC.FluteSpacing)
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltCount2 = fltCount2 - 2
    end -- while end
    local ptE = Polar2D(ptQ, 0.0,  (GetDistance(pt5, pt8) * 0.5))
    local ptF = Polar2D(pt4, 0.0,  Door.Width * 0.5)
    local ptG = Polar2D(ptE,  90.0, StyleC.FluteSpacing)
    local ptH = Polar2D(ptF, 270.0, StyleC.C)

    local ptJ = Polar2D(ptC, 0.0,  (GetDistance(pt5, pt8) * 0.5))
    local ptK = Polar2D(pt1, 0.0,  Door.Width * 0.5)
    local ptM = Polar2D(ptJ, 270.0, StyleC.FluteSpacing)
    local ptN = Polar2D(ptK,  90.0, StyleC.C)
--[[    DebugMarkPoint("ptJ", ptJ, 3, "Debug")
    DebugMarkPoint("ptK", ptK, 3, "Debug")
    DebugMarkPoint("ptM", ptM, 3, "Debug")
    DebugMarkPoint("ptN", ptN, 3, "Debug")
    ]]
    local fltArchCount1 = (GetDistance(ptE, ptH) / StyleC.FluteSpacing)
    local fltArchCount2 = fltArchCount1
    local sig1 = GetDistance(ptG, ptH)
    local sig2 = sig1
    local chord = RadSeg2Chord(rad, sig1)

    while fltArchCount1 > 0 do
      ptC = Polar2D(ptG,   0.0, chord * 0.5)
      ptD = Polar2D(ptG, 180.0, chord * 0.5)
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltArchCount1 = fltArchCount1 - 1.0
      ptG = Polar2D(ptG, 90.0, StyleC.FluteSpacing)
      sig1 = GetDistance(ptG, ptH)
      chord = RadSeg2Chord(rad, sig1)
    end -- while end
    sig2 = GetDistance(ptM, ptN)
    chord = RadSeg2Chord(rad, sig2)
    while fltArchCount2 > 0 do
      ptC = Polar2D(ptM,   0.0, chord * 0.5)
      ptD = Polar2D(ptM, 180.0, chord * 0.5)
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltArchCount2 = fltArchCount2 - 1.0
      ptM = Polar2D(ptM, 270.0, StyleC.FluteSpacing)
      sig2 = GetDistance(ptM, ptN)
      chord = RadSeg2Chord(rad, sig2)
    end -- while end
  end -- if end
...
function DoorStyleE()                                 -- Draws a Door Style E
  ...
  local pt7 = Polar2D(pt6,  90.0, (Door.Height - (StyleE.B + StyleE.C)))
  local pt8 = Polar2D(pt5,  90.0, (Door.Height - (StyleE.B + StyleE.C)))


  local chr = GetDistance(pt7, pt8)
  local dis = GetDistance(pt5, pt8)
  local seg = StyleE.B - StyleE.C
  local rad = ChordSeg2Radius (chr, seg)
  ...
  if (Door.Width > (StyleE.A * 2.0)) and  (Door.Height > (StyleE.B + StyleE.C)) then
    if (Door.Width > (StyleE.A * 2.0)) and (dis >= Door.BitRad) then
        ...
        if StyleE.Fluting == "Vertical" then
          DoorStyleEVertical(pt5, pt6, pt8, rad, seg, chr)
        elseif StyleE.Fluting == "Horizontal" then
          DoorStyleEHorizontal(pt4, pt5, pt6, pt8, rad)
...
-- ====================================================]]
function DoorStyleEVertical(pt5, pt6, pt8, rad, seg, chor)
  local ptA = Polar2D(pt5,   0.0, (GetDistance(pt5, pt6) * 0.5))
  local ptB = Polar2D(pt8,   0.0, (GetDistance(pt5, pt6) * 0.5))
  local ptBa = Polar2D(ptB,  90.0, seg)
  local ptC = ptA
  local ptD = ptB
  local fltCount1 = (GetDistance(pt5, pt6) / StyleE.FluteSpacing) - 1
  local fltCount2 = fltCount1
  local chord = StyleE.FluteSpacing * 2.0
  DrawLine(ptA, ptBa, Door.LayerFlutes)
  local Segment1 = (rad - (0.5 * (math.sqrt((4.0 * rad^2) - chord^2))))
  local Segment2 = (rad - (0.5 * (math.sqrt((4.0 * rad^2) - chord^2))))
  if (1.0 < fltCount1) then
    local Arcstep = seg - Segment1
    while fltCount1 > 1 do
      ptA = Polar2D(ptA, 0.0, StyleE.FluteSpacing)
      ptB = Polar2D(ptB, 0.0, StyleE.FluteSpacing)
      ptBa = Polar2D(ptB, 90.0, Arcstep)
      if chor > chord then
      DrawLine(ptA, ptBa, Door.LayerFlutes)
      else
      ...
      fltCount1 = fltCount1 - 2
      chord = chord + (StyleE.FluteSpacing * 2.0)
      Segment1 = (rad - (0.5 * (math.sqrt((4.0 * rad^2) - chord^2))))
      Arcstep = seg - Segment1
    end -- while end
    Arcstep = seg - Segment2
    chord = StyleE.FluteSpacing * 2.0
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 180.0, StyleE.FluteSpacing)
      ptD = Polar2D(ptD, 180.0, StyleE.FluteSpacing)
      ptBa = Polar2D(ptD, 90.0, Arcstep)
      if chor > chord then
        DrawLine(ptC, ptBa, Door.LayerFlutes)
      else
...
-- ====================================================]]
function DoorStyleEHorizontal(pt4, pt5, pt6, pt8, rad)
  local ptA = Polar2D(pt5,  90.0, (GetDistance(pt5, pt8) * 0.5))
  local ptB = Polar2D(pt6,  90.0, (GetDistance(pt5, pt8) * 0.5))
  local ptC = ptA
  local ptD = ptB
  local fltCount1 = (GetDistance(pt5, pt8) / StyleE.FluteSpacing) - 1
  local fltCount2 = fltCount1
  DrawLine(ptA, ptB, Door.LayerFlutes)
  if (1.0 < fltCount1) then
    while fltCount1 > 1 do
      ptA = Polar2D(ptA, 90.0, StyleE.FluteSpacing)
      ptB = Polar2D(ptB, 90.0, StyleE.FluteSpacing)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      fltCount1 = fltCount1 - 2
    end -- while end
    local ptQ = ptA
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 270.0, StyleE.FluteSpacing)
      ptD = Polar2D(ptD, 270.0, StyleE.FluteSpacing)
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltCount2 = fltCount2 - 2
    end -- while end
    local ptE = Polar2D(ptQ,  0.0, (GetDistance(pt5, pt6) * 0.5))
    local ptF = Polar2D(pt4,  0.0, Door.Width * 0.5)
    local ptG = Polar2D(ptE, 90.0, StyleE.FluteSpacing)
    local ptH = Polar2D(ptF,270.0, StyleE.C)
    local fltArchCount = (GetDistance(ptE, ptH) / StyleE.FluteSpacing)
    local sig = GetDistance(ptG, ptH)
    local chord = RadSeg2Chord(rad, sig)
    while fltArchCount > 0 do
      ptC = Polar2D(ptG,   0.0, chord * 0.5)
      ptD = Polar2D(ptG, 180.0, chord * 0.5)
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltArchCount = fltArchCount - 1.0
      ptG = Polar2D(ptG, 90.0, StyleE.FluteSpacing)
      sig = GetDistance(ptG, ptH)
      chord = RadSeg2Chord(rad, sig)
    end -- while end
  end -- if end
...
function DoorStyleF()                                 -- Draws a Door Style F
  ...
  local pt4 = Polar2D(Doorpt1,  90.0, Door.Height)
  DrawBox(Doorpt1, pt2, pt3, pt4, Door.LayerOFrame)
  local pt5 = Polar2D(Polar2D(Doorpt1, 0.0, StyleF.A), 90.0, StyleF.B)
  local pt6 = Polar2D(pt5,   0.0, (Door.Width - (StyleF.A * 2.0)))
  local pt7 = Polar2D(pt6,  90.0, (Door.Height - (StyleF.B * 2.0)))
  local pt8 = Polar2D(pt5,  90.0, (Door.Height - (StyleF.B * 2.0)))
  local chr = GetDistance(pt7, pt8)
  local seg = StyleF.B - StyleF.C
  local rad = ChordSeg2Radius (chr, seg)
  local bulge =  Radius2Bulge (pt7, pt8, ChordSeg2Radius (chr, seg))
  if (Door.Width > (StyleF.A * 2.0)) and  (Door.Height > (StyleF.B + StyleF.C)) then
    if (Door.Width > (StyleF.A * 2.0)) then
      ...
      local layer = Door.job.LayerManager:GetLayerWithName(Door.LayerIFrame)
      line:AppendPoint(pt5)
      line:ArcTo(pt6, bulge)
      line:LineTo(pt7)
      line:ArcTo(pt8, bulge)
      line:LineTo(pt5)
      layer:AddObject(CreateCadContour(line), true)
      if StyleF.Fluting == "Vertical" then
        DoorStyleFVertical(pt5, pt6, pt8, rad, seg, chr)
      elseif StyleF.Fluting == "Horizontal" then
        DoorStyleFHorizontal(Doorpt1, pt4, pt5, pt6, pt8, rad)
...
-- ====================================================]]
function DoorStyleFVertical(pt5, pt6, pt8, rad, seg, chor)
  local ptA = Polar2D(pt5,   0.0, (GetDistance(pt5, pt6) * 0.5))
  local ptB = Polar2D(pt8,   0.0, (GetDistance(pt5, pt6) * 0.5))
  local ptBa = Polar2D(ptB,  90.0, seg)
  local ptBb = Polar2D(ptA, 270.0, seg)
  local ptC = ptA
  local ptD = ptB
--[[ DebugMarkPoint("pt5", pt5, 3, "Debug")
   DebugMarkPoint("pt6", pt6, 3, "Debug")
   DebugMarkPoint("pt8", pt8, 3, "Debug")
   DebugMarkPoint("ptA", ptA, 3, "Debug")
   DebugMarkPoint("ptB", ptB, 3, "Debug")
   DebugMarkPoint("ptBa", ptBa, 3, "Debug")
   DebugMarkPoint("ptBb", ptBb, 3, "Debug")]]

  local fltCount1 = (GetDistance(pt5, pt6) / StyleF.FluteSpacing)
  local fltCount2 = fltCount1
  local chord = StyleF.FluteSpacing * 2.0
  DrawLine(ptBa, ptBb, Door.LayerFlutes)
  local Segment = (rad - (0.5 * (math.sqrt((4.0 * rad^2) - chord^2))))
  local Segment2 = Segment
  if (1.0 < fltCount1) then
    local Arcstep = seg - Segment
    while fltCount1 > 1 do
      ptA = Polar2D(ptA, 0.0, StyleF.FluteSpacing)
      ptB = Polar2D(ptB, 0.0, StyleF.FluteSpacing)
      ptBa = Polar2D(ptB, 90.0, Arcstep)
      ptBb = Polar2D(ptA, 270.0, Arcstep)
      if chor > chord then
        DrawLine(ptBb, ptBa, Door.LayerFlutes)
      ...
      fltCount1 = fltCount1 - 2
      chord = chord + (StyleF.FluteSpacing * 2.0)
      Segment = (rad - (0.5 * (math.sqrt((4.0 * rad^2) - chord^2))))
      Arcstep = seg - Segment
    end -- while end
    Arcstep = seg - Segment2
    chord = StyleF.FluteSpacing * 2.0
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 180.0, StyleF.FluteSpacing)
      ptD = Polar2D(ptD, 180.0, StyleF.FluteSpacing)
      ptBa = Polar2D(ptD, 90.0, Arcstep)
      ptBb = Polar2D(ptC,270.0, Arcstep)
      if chor > chord then
        DrawLine(ptBa, ptBb, Door.LayerFlutes)
...
-- ====================================================]]
function DoorStyleFHorizontal(pt1, pt4, pt5, pt6, pt8, rad)
  local ptA = Polar2D(pt5,  90.0, (GetDistance(pt5, pt8) * 0.5))
  local ptB = Polar2D(pt6,  90.0, (GetDistance(pt5, pt8) * 0.5))
  local ptC = ptA
  local ptD = ptB
  local fltCount1 = (GetDistance(pt5, pt8) / StyleF.FluteSpacing) - 1
  local fltCount2 = fltCount1
  DrawLine(ptA, ptB, Door.LayerFlutes)
  if (1.0 < fltCount1) then
    while fltCount1 > 1 do
      ptA = Polar2D(ptA, 90.0, StyleF.FluteSpacing)
      ptB = Polar2D(ptB, 90.0, StyleF.FluteSpacing)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      fltCount1 = fltCount1 - 2
    end -- while end
    local ptQ = ptA
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 270.0, StyleF.FluteSpacing)
      ptD = Polar2D(ptD, 270.0, StyleF.FluteSpacing)
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltCount2 = fltCount2 - 2
    end -- while end
    local ptE = Polar2D(ptQ,  0.0, (GetDistance(pt5, pt6) * 0.5))
    local ptF = Polar2D(pt4,  0.0, Door.Width * 0.5)
    local ptG = Polar2D(ptE, 90.0, StyleF.FluteSpacing)
    local ptH = Polar2D(ptF,270.0, StyleF.C)
    local ptJ = Polar2D(ptC,   0.0, (GetDistance(pt5, pt6) * 0.5))
    local ptK = Polar2D(pt1,   0.0, Door.Width * 0.5)
    local ptM = Polar2D(ptJ, 270.0, StyleF.FluteSpacing)
    local ptN = Polar2D(ptK,  90.0, StyleF.C)
    fltArchCount1 = (GetDistance(ptE, ptH) / StyleF.FluteSpacing) -1
    fltArchCount2 = fltArchCount1
    local sig1 = GetDistance(ptG, ptH)
    local sig2 = sig1
    local chord = RadSeg2Chord(rad, sig1)
    while fltArchCount1 > 0 do
      ...
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltArchCount1 = fltArchCount1 - 1.0
      ptG = Polar2D(ptG, 90.0, StyleF.FluteSpacing)
      sig1 = GetDistance(ptG, ptH)
      chord = RadSeg2Chord(rad, sig1)
    end -- while end
    sig2 = GetDistance(ptM, ptN)
    ...
    while fltArchCount2 > 0 do
      ...
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltArchCount2 = fltArchCount2 - 1.0
      ptM = Polar2D(ptM, 270.0, StyleF.FluteSpacing)
      sig2 = GetDistance(ptM, ptN)
      chord = RadSeg2Chord(rad, sig2)
    end -- while end
  end -- if end
...
function DoorStyleG()                                 -- Draws a Door Style G
  ...
  local pt4 = Polar2D(Doorpt1,  90.0, Door.Height)
  DrawBox(Doorpt1, pt2, pt3, pt4, Door.LayerOFrame)
  local ptA = Polar2D(Polar2D(Doorpt1, 0.0, StyleG.A), 90.0, StyleG.B)
  ...
  if (Door.Width > (StyleG.A * 2.0)) and  (Door.Height > (StyleG.B + StyleG.B)) then
    if (Door.Width > (StyleG.A * 2.0) + (StyleG.C * 2.0)) then
      ...
      if StyleG.Fluting == "Vertical" then
        DoorStyleGVertical(Doorpt1, pt5, pt6, pt10, chr)
      elseif StyleG.Fluting == "Horizontal" then
        DoorStyleGHorizontal(Doorpt1, pt11, pt12, pt7, chr)
...
-- ====================================================]]
function DoorStyleGVertical(pt1, pt5, pt6, pt10, chor)
  local distH = GetDistance(pt5, pt6)
  local distV = GetDistance(pt5, pt10)
  local ptA = Polar2D(pt5,    0.0, distH * 0.5) -- Bottom
  local ptB = Polar2D(pt10,   0.0, distH * 0.5) -- Top
  local ptC = ptA
  local ptD = ptB
  local fltCount1 = (distH / StyleG.FluteSpacing) - 1
  local fltCount2 = fltCount1
  DrawLine(ptA, ptB, Door.LayerFlutes)
  if (1.0 < fltCount1) then
    while fltCount1 > 1 do
      ptA  = Polar2D(ptA,   0.0, StyleG.FluteSpacing)
      ptB  = Polar2D(ptB,   0.0, StyleG.FluteSpacing)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      fltCount1 = fltCount1 - 2
    end -- while end
    local ptE = ptA
    local ptJ = Polar2D(ptE,  90.0,  distV * 0.5)
    local ptK = Polar2D(Polar2D(pt1, 0.0,  Door.Width), 90.0,  Door.Height * 0.5)
          ptK = Polar2D(ptK, 180.0,  StyleG.A)
    local ptM = Polar2D(ptK, 180.0,  StyleG.C)
    local ptN = Polar2D(ptJ,   0.0,  StyleG.FluteSpacing)
    fltCount1 = (GetDistance(ptN, ptK) / StyleG.FluteSpacing) + 1
    local sig = StyleG.C - GetDistance(ptN, ptK)
    local chord = RadSeg2Chord(StyleG.C, sig)
    local Hchord = chord * 0.5
    while fltCount1 > 1 do
      ptA = Polar2D(ptN,  90.0,  (distV * 0.5) - Hchord)
      ptB = Polar2D(ptN, 270.0,  (distV * 0.5) - Hchord)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      ptN = Polar2D(ptN,  0.0,  StyleG.FluteSpacing)
      sig = StyleG.C - GetDistance(ptN, ptK)
      Hchord = (RadSeg2Chord(StyleG.C, sig)) * 0.5
      fltCount1 = fltCount1 - 1
    end -- While end
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 180.0, StyleG.FluteSpacing)
      ptD = Polar2D(ptD, 180.0, StyleG.FluteSpacing)
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltCount2 = fltCount2 - 2
    end -- while end
    ptJ = Polar2D(ptC,  90.0, distV * 0.5)
    ptK = Polar2D(pt1,  90.0, Door.Height * 0.5)
    ptK = Polar2D(ptK,   0.0, StyleG.A)
    ptM = Polar2D(ptK,   0.0, StyleG.C)
    ptN = Polar2D(ptJ, 180.0, StyleG.FluteSpacing)
    fltCount1 = (GetDistance(ptN, ptK) / StyleG.FluteSpacing) + 1
    sig = StyleG.C - GetDistance(ptN, ptK)
    chord = RadSeg2Chord(StyleG.C, sig)
    Hchord = chord * 0.5
    while fltCount1 > 1 do
      ptA = Polar2D(ptN,  90.0,  (distV * 0.5) - Hchord)
      ptB = Polar2D(ptN, 270.0,  (distV * 0.5) - Hchord)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      ptN = Polar2D(ptN, 180.0,  StyleG.FluteSpacing)
      sig = StyleG.C - GetDistance(ptN, ptK)
      Hchord = (RadSeg2Chord(StyleG.C, sig)) * 0.5
      fltCount1 = fltCount1 - 1
    end -- While end
...
-- ====================================================]]
function DoorStyleGHorizontal(pt1, pt11, pt12, pt7, chor)
  local distH = GetDistance(pt12, pt7)
  local distV = GetDistance(pt11, pt12)
  local ptA = Polar2D(pt12, 90.0, distV * 0.5) -- Bottom
  local ptB = Polar2D(pt7,  90.0, distV * 0.5) -- Top
  local ptC = ptA
  local ptD = ptB

  local fltCount1 = (distV / StyleG.FluteSpacing) - 1
  local fltCount2 = fltCount1
  DrawLine(ptA, ptB, Door.LayerFlutes)
  if (1.0 < fltCount1) then
    while fltCount1 > 1 do
      ptA  = Polar2D(ptA, 90.0, StyleG.FluteSpacing)
      ptB  = Polar2D(ptB, 90.0, StyleG.FluteSpacing)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      fltCount1 = fltCount1 - 2
    end -- while end
    local ptJ = Polar2D(ptA,  0.0, distH * 0.5)
    local ptK = Polar2D(pt1,   0.0, Door.Width * 0.5)
          ptK = Polar2D(ptK,  90.0, Door.Height)
          ptK = Polar2D(ptK, 270.0, StyleG.B)
    local ptM = Polar2D(ptK, 270.0, StyleG.C)
    local ptN = Polar2D(ptJ,  90.0, StyleG.FluteSpacing)
    fltCount1 = (GetDistance(ptN, ptK) / StyleG.FluteSpacing) + 1
    local sig = StyleG.C - GetDistance(ptN, ptK)
    local chord = RadSeg2Chord(StyleG.C, sig)
    local Hchord = chord * 0.5
    while fltCount1 > 1 do
      ptA = Polar2D(ptN,   0.0,  (distH * 0.5) - Hchord)
      ptB = Polar2D(ptN, 180.0,  (distH * 0.5) - Hchord)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      ptN = Polar2D(ptN,  90.0,  StyleG.FluteSpacing)
      sig = math.abs(GetDistance(ptN, ptK) - StyleG.C)
      Hchord = (RadSeg2Chord(StyleG.C, sig)) * 0.5
      fltCount1 = fltCount1 - 1
    end -- While end
    while fltCount2 > 1 do
      ptC = Polar2D(ptC, 270.0, StyleG.FluteSpacing)
      ptD = Polar2D(ptD, 270.0, StyleG.FluteSpacing)
      DrawLine(ptC, ptD, Door.LayerFlutes)
      fltCount2 = fltCount2 - 2
    end -- while end
    ptJ = Polar2D(ptC,   0.0, distH * 0.5)
    ptK = Polar2D(pt1,   0.0, Door.Width * 0.5)
    ptK = Polar2D(ptK,  90.0, StyleG.B)
    ptM = Polar2D(ptK,  90.0, StyleG.C)
    ptN = Polar2D(ptJ, 270.0, StyleG.FluteSpacing)
    fltCount2 = (GetDistance(ptN, ptK) / StyleG.FluteSpacing) + 1
    sig = math.abs(GetDistance(ptN, ptK) - StyleG.C)
    chord = RadSeg2Chord(StyleG.C, sig)
    Hchord = chord * 0.5
    while fltCount2 > 1 do
      ...
      ptB = Polar2D(ptN, 180.0, (distH * 0.5) - Hchord)
      DrawLine(ptA, ptB, Door.LayerFlutes)
      ptN = Polar2D(ptN,  270.0,  StyleG.FluteSpacing)
      sig = math.abs(GetDistance(ptN, ptK) - StyleG.C)
      Hchord = (RadSeg2Chord(StyleG.C, sig)) * 0.5
...
DialogWindow.myHtmlE = [[<!DOCTYPE html><head><title>Door Style</title> ]] .. DialogWindow.Style ..[[</head><body><table><tr><td><span class="h1-c">]] .. DialogWindow.Eimage ..[[</span></td><td colspan="2" valign="top"><table><tr><td colspan="2" class="h2-l" id="Door.Style">Door Style Name</td></tr>
<tr><td class="h2-r">A</td><td class="h1-l"><input type="text" id="Door.A" size="10" maxlength="10" /></td></tr>
<tr><td class="h2-r">B</td><td class="h1-l"><input type="text" id="Door.B" size="10" maxlength="10" /></td></tr>
<tr><td class="h2-r">C</td><td class="h1-l"><input type="text" id="Door.C" size="10" maxlength="10" /></td></tr>
</table></td></tr></table><table><tr><td><span class="h1-l"> <select name="Door.Fluting" id="Door.Fluting"> </select> </span></td><td><span class="h2-r">Flute Spacing</span></td><td><span class="h1-l"> <input type="text" id="Door.FluteSpacing" size="10" /> </span></td></tr></table><table><tr><td colspan="3"><hr></td></tr><tr><td align="center" class="FootColumn3" ><input id="ButtonOK" class="FormButton" name="ButtonOK" type="button" value="OK"></td><td align="center" class="FootColumn4" ><input id="ButtonCancel" class="FormButton" name="ButtonCancel" type="button" value="Cancel"></td></tr></table></body></html>]]
...
DialogWindow.myHtmlF = [[<!DOCTYPE html><head><title>Door Style</title> ]] .. DialogWindow.Style ..[[</head><body><table><tr><td><span class="h1-c">]] .. DialogWindow.Fimage ..[[</span></td><td colspan="2" valign="top"><table><tr><td colspan="2" class="h2-l" id="Door.Style">Door Style Name</td></tr>
<tr><td class="h2-r">A</td><td class="h1-l"><input type="text" id="Door.A" size="10" maxlength="10" /></td></tr>
<tr><td class="h2-r">B</td><td class="h1-l"><input type="text" id="Door.B" size="10" maxlength="10" /></td></tr>
<tr><td class="h2-r">C</td><td class="h1-l"><input type="text" id="Door.C" size="10" maxlength="10" /></td></tr>
</table></td></tr></table><table><tr><td><span class="h1-l"> <select name="Door.Fluting" id="Door.Fluting"> </select> </span></td><td><span class="h2-r">Flute Spacing</span></td><td><span class="h1-l"> <input type="text" id="Door.FluteSpacing" size="10" /> </span></td></tr></table><table><tr><td colspan="3"><hr></td></tr><tr><td align="center" class="FootColumn3" ><input id="ButtonOK" class="FormButton" name="ButtonOK" type="button" value="OK"></td><td align="center" class="FootColumn4" ><input id="ButtonCancel" class="FormButton" name="ButtonCancel" type="button" value="Cancel"></td></tr></table></body></html>]]
...
<body>
<table class="TableSize600">
  <tr>
    <td colspan="4">
    <p>This Gadget draws cabinet doors based on user imputs (Height, Width, style, etc...) Doors are produced by Batch Mode (Reads a CSV file) or Manunal Mode.</p>
    </td>
  </tr>
  <tr>
     <td colspan="4"><hr></td>
  ...
  <tr>
    <td><span class="h1-c">
      <select name="DoorStyle" id="DoorStyle">
        <option value="StyleA">Door Style A</option>
        <option value="StyleB">Door Style B</option>
        <option value="StyleC">Door Style C</option>
        <option value="StyleD">Door Style D</option>
        <option value="StyleE">Door Style E</option>
        <option value="StyleF">Door Style F</option>
        <option value="StyleG">Door Style G</option>
      </select>
      </span></td>
    <td><span class="h1-l"><span class="h1-c">
      <input name="InquiryStyle" type="button" class="LuaButton" id="InquiryStyle" value="Edit Style">
      </span></span></td>
    <td><input id="InquiryAbout" class="LuaButton" name="InquiryAbout" type="button" value=" About "></td>
    <td class="h2-r"><input name="InquiryDefault" type="button" class="LuaButton" id="InquiryDefault" value="Default Settings"></td>
  </tr>
</table>
<table class="TableSize600">
  <tr>
  ...
  <tr>
    <td colspan="6" class="1abel-l">Manual Mode</td>
  </tr>
  <tr>
  ...
  <tr>
    <td class="h1-c"><input type="text" id="Door.X" size="10" maxlength="10" /></td>
    <td class="h1-c"><input type="text" id="Door.Y" size="10" maxlength="10" /></td>
    <td class="h1-c"><input type="text" id="Door.Count" size="10" maxlength="10" /></td>
    <td class="h1-c"><input type="text" id="Door.Height" size="10" maxlength="10" /></td>
    <td class="h1-c"><input type="text" id="Door.Width" size="10" maxlength="10" /></td>
...
<table>
  ...
  <tr>
    <td colspan="3" class="lable-l">Batch Mode</td>
  </tr>
  <tr>
    ...
    <td width="15" align="right"><input id="ButtonOK" class="FormButton" name="ButtonOK" type="button" value=" Run "></td>
  </tr>
</table>
<table class="TableSize600" id="HelpMessageOKCancel">
  <tr>
    <td colspan="3"><hr></td>
  </tr>
  <tr>
    <td class="FootColumn1" ><button class="FormButton" onClick="window.open(']].. DialogWindow.ProjectSDK .. [[');"> Help </button></td>
    <td class="FootColumn2" id="Door.Alert">.</td>
    <td class="h1-rPx" ><input id="ButtonCancel" class="FormButton" name="ButtonCancel2" type="button" value="Cancel"></td>
  </tr>
</table>
