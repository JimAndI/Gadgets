...
g_version = "modified by Jeff Elliott 113014"
g_title = "Create Chamfer Toolpath In and Out"
g_width = 800
g_height = 625
g_html_file = "Chamfer In and Out.htm"
...
------------ADDED--------
  ...
  local offset_group = single_contour_group:OffsetWithOpenVectors(horizontalOff, horizontal_offset, 1, false)
...
function GetSpanIndexParam(contour, cur_z)
  ...
  -- Assuming that our contour has been smashed.
...
| given span. Assumes that the contour has
...
function CalculateStraightCornerVectors(contour, angle, depth, start_z, tool)
  ...
  -- we construct a straight line back at given angle
  local corners = {}
  local pos =  contour:GetHeadPosition()
  local span
  while  pos ~= nil do
...
function main(script_path)
  ...
  if (selected_contours.IsOpen) then
    DisplayMessageBox("Please select a closed contour.")
    return false
  end
  ...
  -- math.ceil - returns the integer no greater than the given value
  -- > = math.ceil(0.5)
  -- 1
  -- num_offsets = the total horizontal distance to be cut divided by the tool stepover
  ...
  while p_selected ~= nil do
    ...
    if current_contour.IsOpen then
      ...
      -- 
      offset_contours = current_group:Offset(signed_offset_dist, single_dist, num_offsets , true)
      -- ORIGINAL offset_contours = current_group:OffsetWithOpenVectors(signed_offset_dist, single_dist, num_offsets , false)
      --***************
      --the bool in MakeOffsetsSquare should vary depending on offset direction 
      -- offset_out - bool - if true the offset operation was an offset outwards
      if not options.chamfer_in then
        offset_contours:MakeOffsetsSquare(single_dist , true, 0.5*options.tool.ToolDia)
      else
    ...
    --local current_z = options.finish_z
    ...
    while p_cntr ~= nil do
      ...
      -- end
      -- same for both in and out -- I think
      cntr, p_cntr = offset_contours:GetNext(p_cntr)
...
function DisplayDialog( script_path, options )
  ...
  ----- Added to round angle-------------------------
  --dialog:AddDoubleField("ChamferAngle", math.deg(options.angle))
  local raw_angle = math.deg(options.angle)
  -- call function round(what, precision)
  local rounded_angle = round(raw_angle, 1)
  -- MessageBox("Angle  " .. rounded_angle )
  dialog:AddDoubleField("ChamferAngle", rounded_angle)
----- Added End -------------------------	
...
function OnLuaButton_XXXX(button_name, dialog)
  ...
  --MessageBox("User pressed a LuaButton  " .. chamfer_type_index)
