...
--|
--| Easy Euro Hinge Maker is written by Jim Anderson of Houston Texas 2020
]]
-- ===================================================]]
--[[
Do List:
...
DialogWindow.SDK         = "https://jimandi.com/SDKWiki/index.php?title=Easy_32mm_System"
DialogWindow.SDKLayer    = "https://jimandi.com/SDKWiki/index.php?title=Easy_32mm_System-Layers"
DialogWindow.SDKMilling  = "https://jimandi.com/SDKWiki/index.php?title=Easy_32mm_System-Milling"
DialogWindow.SDKHardware = "https://jimandi.com/SDKWiki/index.php?title=Easy_32mm_System-Hardware"

-- ===================================================]]
function REG_ReadRegistry() -- Read from Registry values
  local RegistryRead         = Registry(Panel.RegName)
  Panel.Height               = RegistryRead:GetDouble("Panel.Height",               24.000)
  Panel.Width                = RegistryRead:GetDouble("Panel.Width",                12.000)
  ...
  Panel.DrillOrMillScrews    = RegistryRead:GetString("Panel.DrillOrMillScrews",    "Pocketing")
  Panel.ProfileBitDiameter   = RegistryRead:GetDouble("Panel.ProfileBitDiameter",   0.250)
  Panel.PinPocketingDiameter = RegistryRead:GetDouble("Panel.PinPocketingDiameter", 0.125)
  Panel.PinDrillingDiameter  = RegistryRead:GetDouble("Panel.PinDrillingDiameter",  0.500)
...
function REG_WriteRegistry()                            -- Write to Registry values
  ...
  local RegValue = RegistryWrite:SetString("Panel.InquiryAboutXY",         Panel.InquiryAboutXY)
        ...
        RegValue = RegistryWrite:SetString("Panel.DrillOrMillScrews",      Panel.DrillOrMillScrews)
        RegValue = RegistryWrite:SetDouble("Panel.ProfileBitDiameter",     Panel.ProfileBitDiameter)
        RegValue = RegistryWrite:SetDouble("Panel.PinPocketingDiameter",   Panel.PinPocketingDiameter)
        RegValue = RegistryWrite:SetDouble("Panel.PinDrillingDiameter",    Panel.PinDrillingDiameter)
        RegValue = RegistryWrite:SetDouble("Panel.PartGap",                Panel.PartGap)
        RegValue = RegistryWrite:SetDouble("Panel.Width",                  Panel.Width)
        RegValue = RegistryWrite:SetDouble("Panel.Height",                 Panel.Height)
...
function OnLuaButton_InquiryMilling()
  local dialog = HTML_Dialog(true, DialogWindow.myHtmlMilling, 658, 187, "Milling Settings")
  dialog:AddDoubleField("Panel.ProfileBitDiameter",     Panel.ProfileBitDiameter)
  dialog:AddDropDownList("Panel.DrillOrMillScrews",     Panel.DrillOrMillScrews)
  dialog:AddDoubleField("Panel.PinPocketingDiameter",   Panel.PinPocketingDiameter)
  dialog:AddDoubleField("Panel.PinDrillingDiameter",    Panel.PinDrillingDiameter)
  dialog:AddDoubleField("Panel.PartGap",                Panel.PartGap)
  if dialog:ShowDialog() then
    Panel.ProfileBitDiameter   = dialog:GetDoubleField("Panel.ProfileBitDiameter")
    Panel.DrillOrMillScrews    = dialog:GetDropDownListValue("Panel.DrillOrMillScrews")
    Panel.PinPocketingDiameter = dialog:GetDoubleField("Panel.PinPocketingDiameter")
    Panel.PinDrillingDiameter  = dialog:GetDoubleField("Panel.PinDrillingDiameter")
    ...
    if Panel.DrillOrMillScrews == "Drilling" then
      Panel.dialog:UpdateTextField("DrillState",  "Drilling")
    else
      Panel.dialog:UpdateTextField("DrillState",  "Pocketing")
    end
    REG_WriteRegistry()
  end
  return  true
...
function InquiryPanel(Header)
    Panel.dialog = HTML_Dialog(true, DialogWindow.myHtmlPanel, 605, 568,  Header .. " (Ver:" .. Panel.VerNumber.. ")  " .. Panel.UnitDisplay) ;
    if Panel.DrillOrMillScrews == "Drilling" then
      Panel.dialog:AddTextField("DrillState",  "Drilling")
    else
      Panel.dialog:AddTextField("DrillState",  "Pocketing")
    end
    Panel.dialog:AddDoubleField("Panel.Height",       Panel.Height)
    ...
    Panel.dialog:AddLabelField("ToolNameLabel3", "Not Selected")
    Panel.dialog:AddToolPicker("ToolChooseButton3", "ToolNameLabel3", Panel.Tool_ID3)
    Panel.dialog:AddToolPickerValidToolType("ToolChooseButton3", Tool.THROUGH_DRILL)
    if  Panel.dialog:ShowDialog() then
      Panel.Width       = Panel.dialog:GetDoubleField("Panel.Width")
      Panel.Height      = Panel.dialog:GetDoubleField("Panel.Height")
      ...
      Panel.MillTool2   = Panel.dialog:GetTool("ToolChooseButton2")  -- Pocketing
      Panel.MillTool3   = Panel.dialog:GetTool("ToolChooseButton3")  -- Drilling
...
function DrawPanel(pt1)
  ...
  local pt3   = Polar2D(pt2, Panel.HAng, Panel.Height)
...
-- ===================================================]]
  if Panel.Inquiry then
    ...
    if Panel.DrillOrMillScrews == "Pocketing" then
  ...
  -- select all closed vectors on the layer
    if not SelectVectorsOnLayer(layer, selection, true, false, true) then
     ...
     -- Create tool we will use to machine vectors
      local tool = Tool(Panel.MillTool3.Name, Tool.THROUGH_DRILL)       -- BALL_NOSE, END_MILL, VBIT, THROUGH_DRILL

      tool.InMM = Panel.MillTool3.InMM -- tool_in_mm
      tool.ToolDia = Panel.MillTool3.ToolDia -- tool_dia
      tool.Stepdown = Panel.MillTool3.Stepdown -- tool_stepdown
      tool.Stepover = Panel.MillTool3.ToolDia * 0.25
      tool.RateUnits = Panel.MillTool3.RateUnits  -- Tool.MM_SEC  -- MM_SEC, MM_MIN, METRES_MIN, INCHES_SEC, INCHES_MIN, FEET_MIN
      tool.FeedRate = Panel.MillTool3.FeedRate  -- 30
      tool.PlungeRate = Panel.MillTool3.PlungeRate   -- 10
      tool.SpindleSpeed = Panel.MillTool3.SpindleSpeed    -- 20000
      tool.ToolNumber = Panel.MillTool3.ToolNumber       -- 1
...
function DrawHoles(pt, hand)
  ...
  Panel.HoleCount = math.floor(Panel.Height / Panel.HoleSpacing)
  Panel.HoleDist  = (Panel.Height - (Panel.HoleSpacing * Panel.HoleCount))  * 0.500
  Panel.BackHoleDist = math.floor((Panel.Width - Panel.FrontDist) / Panel.HoleSpacing) * Panel.HoleSpacing
  if hand == "L" then
    if Panel.Orientation == "Horizontal" then
      ...
      for _ = Panel.HoleCount + 1, 1, -1 do
        ...
        wpt1 = Polar2D(wpt1,180.00, Panel.HoleSpacing)
