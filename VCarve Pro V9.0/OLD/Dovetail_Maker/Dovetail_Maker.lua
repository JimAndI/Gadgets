...
--[[
Create Dovetail Vectors and Toolpaths by Jim Anderson of Houston, Texas. (Mar 2015)
Dovetail Toolpaths program is very heavily based on 
code from Vectric's core software SDK, Brian Moran and many other great developers.
]]

--Global variables
g_PinMaterialThickness 	= 0.750
g_TailMaterialThickness = 0.750
g_MaterialLength 		= 4.000
g_BitDiameter 			= 0.625
g_BitBackcutClearance 	= 0.010
g_BitAngleDegrees 		= 7.00
g_SideStepCut			= g_BitDiameter*SIN(math.rad(g_BitAngleDegrees))
g_SideStepBackCut		= g_SideStepCut + g_BitBackcutClearance
g_MinDoveBitCut			= g_SideStepBackCut + g_SideStepCut 
g_PinCount 				= 3
g_PinCenters 			= g_MaterialLength/g_PinCount 
g_FisrtPinCenter 		= g_PinCenters * 0.50
g_TailCenterOffset 		= g_FisrtPinCenter * 0.50
g_PinWidth 				= g_SideStepBackCut + g_TailCenterOffset
g_offset 				= 0
g_ToolSize1 			= 0.125
g_ToolSize2 			= 0.125
g_layer_name0 			= "Tail_Clarance_Layer"
g_layer_name1 			= "Tail_DoveBit_Layer"
g_layer_name2 			= "Pins_Layer"
g_units  				= " inches"
g_toolpath_name0 		= "Tails Clearance Toolpath"
g_toolpath_name1 		= "Tails Dovetail Toolpath"
g_toolpath_name2 		= "Pins Toolpath"
g_tool1 				= nil
g_tool2 				= nil
g_orantation 			= "Y"
g_direction1 			= 0
...
g_direction4 			= 0
g_bulge 				= 1.0
g_default_tool_name1 	= ""
g_default_tool_name2 	= ""
...
--[[
 ...
 from a know point with angle and distance. You do not have to pull the 
 2D point apart and reconstruct the 2Dpoint back together
 ==============================================================================
]]
...
function GetMaterialSettings()
  ...
  if mtl_block.Height == mtl_block.Width then
      MessageBox("Error! Material block cannot be square")
  end
...
function PinPoints(pt1, LayerName)
...
-- Build the finger box points
function TailClearancePoints(pt1, LayerName)
...
-- Build the finger box points
function TailPoints(pt1, LayerName)
...
-- ______________________________________________________________________________
-- Build the Tails joints
function CreatTailsPaths()

  local BasePoint = Point2D(0,0)
  BasePoint = Polar2D(BasePoint, math.rad(g_direction1), g_FingerSize )  
  
  -- Loop the Tails for each fingers on part 1  
  for n=1, g_NoFingers0 do
    BasePoint = TailClearancePoints(BasePoint, g_layer_name0)
  ...
  for n=1, g_NoFingers1 do
    BasePoint = TailPoints(BasePoint, g_layer_name1)
...
-- ______________________________________________________________________________
-- Build the Pins joints
function CreatPinPaths()

  local BasePoint = Point2D(0,0)
  BasePoint = Polar2D(BasePoint, math.rad(g_direction1), g_FingerSize)  
  
  -- Loop the Pins for each fingers on part 1  
  for n=1, g_NoFingers2 do
    BasePoint = PinPoints(BasePoint, g_layer_name2)
...
-- ______________________________________________________________________________
function CreatePocketToolpath(job, layer_name, toolpath_name, material_thickness)
  ...
  local pocket_data                 = PocketParameterData()
        ...
        pocket_data.CutDepth        = material_thickness
...
function LoadDialog(job, script_path)
	local registry = Registry("DovetailToolpath")
	local html_path = "file:" .. script_path .. "\\Dovetail_Maker.htm"
	...
	dialog:AddDoubleField("TailBoradThickness",                	g_TailBoradThickness)
	dialog:AddDoubleField("PinBoradThickness",                	g_PinBoradThickness)
	dialog:AddDoubleField("BackCutClearance",                	g_BackCutClearance)
	dialog:AddDoubleField("PinCount",                			g_PinCount)
	dialog:AddDoubleField("BitAngle",                			g_BitAngle)
	...
	dialog:AddToolPicker("ToolChooseButton1", "ToolNameLabel1", 	g_default_tool_name1)
	dialog:AddToolPicker("ToolChooseButton2", "ToolNameLabel2", 	g_default_tool_name2)
	dialog:AddToolPickerValidToolType("ToolChooseButton1",     		Tool1.END_MILL)
	dialog:AddToolPickerValidToolType("ToolChooseButton2",     		Tool2.END_MILL)
	...
	local tool1 = dialog:GetTool("ToolChooseButton1")
	if tool1 == nil then
		MessageBox("No tool selected!")
		return 2
		-- 2 value will return user to Dovetail dialog
	end
	
	local tool2 = dialog:GetTool("ToolChooseButton2")
	if tool2 == nil then
		MessageBox("No tool selected!")
		return 2
		-- 2 value will return user to Dovetail dialog
	...
	g_tool1 					= tool1
	g_tool2 					= tool2
	g_PinMaterialThickness 		= tonumber(dialog:GetDoubleField("PinMaterialThickness"))
	g_TailMaterialThickness 	= tonumber(dialog:GetDoubleField("TailMaterialThickness"))
	g_MaterialLength 			= tonumber(dialog:GetDoubleField("MaterialLength"))
	g_PinCount					= tonumber(dialog:GetDoubleField("PinCount"))
	g_default_tool_name1 		= tool1.Name
	g_default_tool_name2 		= tool2.Name
	...
	if tool1.ToolDia > g_FingerSize then
	    MessageBox("The bit is too larger for the number of Fingers")
		return 2
		-- 2 value will return user to Dovetail dialog
	end

	if tool2.ToolDia > g_FingerSize then
	    MessageBox("The bit is too larger for the number of Fingers")
		return 2
		-- 2 value will return user to Dovetail dialog
	end
...
function main(script_path)
	...
	-- Building the vectors for the Pin paths
	CreatPinCutPaths() 
	
	-- Building the vectors for the Tails Clearance Path
	CreatClearancePaths() 
	
	-- Building the vectors for the Tails Path
	CreatTailPaths() 
	
	-- Build the tool paths for Tails Clarence joint
	CreateTailClarenceToolpath(job, g_layer_name0, g_toolpath_name0)
	
	-- Build the tool paths for Tails joint
	CreateTailToolpath(job, g_layer_name1, g_toolpath_name1)
	
	-- Build the tool paths for Pins joint
	CreatePinsToolpath(job, g_layer_name2, g_toolpath_name2)
