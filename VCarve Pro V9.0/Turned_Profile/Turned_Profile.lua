...
function CreateOffsetWithOpenVectors(contour, offset)
  ...
  if contour == nil then
    MessageBox("Null contour passed in")
    return nil
  end
  ...
  local offset_contour = offset_group:RemoveHead()
...
function OffsetAndClipContourForTool(contour, tool)
  ...
  -- Get the tool radius
  local tool_radius = 0.5 * tool.ToolDia
  
  -- Offset the contour
  local offset_contour = CreateOffsetWithOpenVectors(contour, -tool_radius)
...
--[[  -------------- CreateClippedProfileAtPassDepth --------------------------------------------------
|
|  Given an original profile clip it to the specified depth
|
|  Parameters:
|   profile           -- The profile to clip
|	profile_depth     -- The starting depth of the profile
|   pass_depth        -- The pass depth
|   start_depth       -- The start depth
|     
|
|  Return Values:
|   Returns the clipped profile
|
]]
function CreateClippedProfileAtPassDepth(profile, profile_depth, pass_depth, start_depth)
  
  -- Function that takes a point from the original contour and transforms it 
  -- into 'material block' space
  local TransformPoint = function(point)
    local start_pt = profile.StartPoint2D
    local x = (point.x - start_pt.x)
    local z = (start_pt.y - point.y) + profile_depth
    return x, z
  end
  
  -- Set up the result 
  local clipped = Contour(0.0)
  if clipped == nil then
    return nil
  end
  
  -- Append points/lines at relative depths
  local AppendPointAtRelativeDepth = function(x, depth)
    clipped:AppendPoint(x, 0, start_depth - depth)
  end
  local AppendLineAtRelativeDepth = function(x, depth)
    clipped:LineTo(x, 0, start_depth - depth)
  end
  ...
  if above then
    AppendPointAtRelativeDepth(0, profile_depth)
  else
    AppendPointAtRelativeDepth(0, pass_depth)
  end
  ...
  while pos ~= nil do
    ...
    if above then
      if z1 > pass_depth then
        ...
        local end_pt = clipped.EndPoint3D
        local x0 = end_pt.x
        local z0 = start_depth - end_pt.z -- Need to use the absolute rather than the relative depth
        x1 = x0 + (x1 - x0) * (pass_depth - z0) / (z1 - z0)
        z1 = pass_depth
        above = false
      end
      AppendLineAtRelativeDepth(x1, z1)
    else
      ...
      if z1 < pass_depth then
        ...
        AppendLineAtRelativeDepth(x, pass_depth)
        -- Add the end of the line segment
        AppendLineAtRelativeDepth(x1, z1)
        above = true
      end
    end
  ...
  if not above then
    local x = profile.EndPoint2D.x - profile.StartPoint2D.x
    AppendLineAtRelativeDepth(x, pass_depth)
  end
...
function CreateToolpathPassContour(profile, blc, height, step_over)
  
  -- Clone the profile and reverse it
  local left_profile = profile:Clone()
  if left_profile == nil then
    return nil
  end
  ...
  local y = 0
  while y < height do
    local translation = Vector2D(blc.x, blc.y + y)
    local xform = TranslationMatrix2D(translation)
    local template = right and profile or left_profile
    local next_segment = template:Clone()
    if next_segment == nil then 
      return nil
    end
    next_segment:Transform(xform)
    if not pass_contour:AppendContour(next_segment) then
      return nil
    end
    ...
    if y < height then
      ...
      pass_contour:LineTo(end_pt.x, blc.y + y, end_pt.z)
    end
    right = not right
  end
...
function CreateWrapYToolpathContourGroup(contour, bounds, thickness, options)
  ...
  local return_length = return_span:GetLength(0.01)
  local profile_depth = thickness - return_length
  ...
  -- Get the offset and clipped contour
  local offset_contour = OffsetAndClipContourForTool(contour, tool)
  if offset_contour == nil then
    return nil
  end
  ...
  local min_height, max_height = GetWrapYContourHeightRange(offset_contour)
  min_height = min_height + return_length
  max_height = max_height + return_length
  ...
  -- Compute the step over
  local scale_factor = thickness / max_height
  scale_factor = math.min(scale_factor, 2.0)
  scale_factor = math.max(scale_factor, 1.0)
  local step_over = scale_factor * tool.Stepover
  
  -- If we're using multiple passes use the tool stepdown otherwise just step down 
  -- by the material thickness
  local step_down = options.use_multiple_passes and tool.Stepdown or thickness
  local max_depth = thickness - min_height
  local passes = math.ceil(max_depth / step_down)
  
  -- Set up the toolpath group
  local toolpath_group = ContourGroup(true)
  if toolpath_group == nil then
    return nil
  end
  
  -- Compute the bottom left hand corner of each pass
  local start_pt = contour.StartPoint2D
  local blc = Point2D(start_pt.x, bounds.BLC.y)
  
  -- Loop over the number of passes
  for pass = 1, passes do
    local pass_depth = pass * step_down
    -- Note the start_depth is always zero
    local pass_profile = CreateClippedProfileAtPassDepth(offset_contour, profile_depth, pass_depth, 0.0)
    if pass_profile == nil then
      return nil
    end
    local pass_contour = CreateToolpathPassContour(pass_profile, blc, bounds.YLength, step_over)
    -- Append this to the group
    toolpath_group:AddTail(pass_contour)
  end
...
function DisplayDialog(options, job, script_path)
  ...
  options.window_height = dialog.WindowHeight
	...
	if options.toolpath_name == nil then
...
<table border="0" width="100%">
	...
	<tr>
		...
		<td align="left">
		<input id="UseMultiplePasses" name="UseMultiplePasses" type="checkbox" value="" checked>Use multiple passes - A clearance will be made at each pass depth based on the selected tools Pass Depth
		</td>
	</tr>
</table>
