...
function main(script_path)
  ...
  local id = component_manager:AddRelief(relief, CombineMode.Add, "03 Cloning And Combine Modes")
  
  -- Get the component back by its id
  local component = component_manager:FindObjectWithId(id)
  
  -- Clone the component - this is effectively a shallow copy
  -- and change the combine mode to merge heighest
  local clone = component_manager:CloneObjectWithId(id, false)
  clone.CombineMode = CombineMode.MergeHeighest
  
  -- Transform the components
  local transform_vector = Vector2D(1.25, 2.5)
  local xform = TranslationMatrix2D(transform_vector)
  component:Transform(xform, true)
  
  transform_vector = Vector2D(3.75, 2.5)
  xform = TranslationMatrix2D(transform_vector)
