...
-- Global Variables --
  ...
  Stair.Version     = "12.0"                             -- Application Build Version
  ...
  Stair.AppVer = GetAppVersion()
  Stair.DialogLoop = true

  if Stair.AppVer > 10 then
    Stair.Tool = [[<table width="100%" border="0"><tr><td colspan="5" nowrap="nowrap" class="ToolNameLabel"><hr></td></tr><tr><td colspan="4" nowrap="nowrap" class="ToolNameLabel"><span width="80%" id = "ToolNameLabel">Tool Name</span></td><td width="10%" align="right"><input type = "button" name = "ToolChooseButton" id = "ToolChooseButton" value = "Select" class = "ToolPicker"/></td>
  </tr></table>]]
  ...
  Tool_ID:LoadDefaults("My Toolpath", "")
    ...
    Stair.Tool = [[  <table width="100%" border="0"><tr><td nowrap="nowrap" class="ToolNameLabel"><hr></td></tr><tr><td nowrap="nowrap" class="alert">This Gadget does not support Older Tool Databse Formats</td></tr></table>]]
  end
 -- Stair.Help = script_path .. "\\Help6.32.pdf"
...
function TrigIt()                                       -- Calulates Right Triangle
  ...
  elseif (Trig.Opp >  0.0) and (Trig.Hyp >  0.0) then -- Base and Bevel
    ...
    DisplayMessageBox("TrigIt Error: Triangle values did not match requirements: \n" ..
        " Trig.A     =  " .. tostring(Trig.A) .. " \n" ..
        " Trig.B     =  " .. tostring(Trig.B) .. " \n" ..
        " Trig.C     =  " .. tostring(Trig.C) .. " \n" ..
        " Trig.Opp   =  " .. tostring(Trig.Opp) .. " \n" ..
...
function CheckDialogValues()                            -- Look at what the User inputed.
 ...
 -- end
  if Stair.RiserThickness  == 0.0 then
    Stair.DialogLoop = true
    DisplayMessageBox("Error: Incorrect Riser Thickness value")
  end
  if Stair.TreadThickness  == 0.0 then
    Stair.DialogLoop = true
    DisplayMessageBox("Error: Incorrect Tread Thickness value")
  end
  if Stair.StepCount  == 0.0 then
    Stair.DialogLoop = true
    DisplayMessageBox("Error: Incorrect Step Count value")
  end
  if Stair.StringerWidth  == 0.0 then
    Stair.DialogLoop = true
    DisplayMessageBox("Error: Incorrect Stringer Width value")
  end
  if Stair.RunDim  == 0.0 then
    Stair.DialogLoop = true
    DisplayMessageBox("Error: Incorrect Stair Run value")
  end
  if Stair.RiseDim  == 0.0 then
    Stair.DialogLoop = true
    DisplayMessageBox("Error: Incorrect Stair Rise value")
  end
  if Stair.StepRise  == 0.0 then
    Stair.DialogLoop = true
    DisplayMessageBox("Error: Incorrect Riser alue")
  end
  if Stair.StepRun  == 0.0 then
    Stair.DialogLoop = true
    DisplayMessageBox("Error: Incorrect Step value")
  end
   if  AllTrim(Stair.TreadLayer)  == "" then
    Stair.DialogLoop = true
    DisplayMessageBox("Error: Incorrect Riser Tread Layer value")
  end
  if AllTrim(Stair.RiserLayer)  == "" then
    Stair.DialogLoop = true
    DisplayMessageBox("Error: Incorrect Riser Riser Layer value")
  end
  if  AllTrim(Stair.StringerLayer)  == "" then
    Stair.DialogLoop = true
    DisplayMessageBox("Error: Incorrect Stringer Layer value")
  end
  if Stair.WriteBOM  == 1 then
    if Stair.BOMWritePath  == 0.0 then
       Stair.DialogLoop = true
       DisplayMessageBox("Error: Incorrect BOM Path value")
    end
  end
end -- function end
-- ===================================================]]
...
--| Video 12: Jan 2020 - Error Trapping an OnLuaButton_ButtonCalulate(dialog)
--]]
-- Localized Variables --

  local Layer = ""   -- establishes a variable named layer
  local WPrad = 0.0  -- establishes a variable named WPrad
