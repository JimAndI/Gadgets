...
Tool_ID3:LoadDefaults("My Toolpath3", "")
  ...
  function CarrierMath()
    ...
    Carrier.SideHight     = Carrier.BottleHight * 0.4
    ...
    Carrier.EndHeight     = Carrier.BottleHight
    -- ============]]
    Carrier.OneSide       = Carrier.Count * 0.5
    ...
    Carrier.HandleWidht   = (Carrier.EndHeight - Carrier.SideHight ) + (Carrier.MaterialBlockThickness * 0.5)
    ...
    -- ==================]]
    Carrier.g_pt1a =  Polar2D(Point2D(0.0, 0.0), 270.0,   Carrier.EndHeight + (Carrier.SideHight * 0.5) + Carrier.PartGap)
    ...
    Carrier.g_pt4a =  Polar2D(Point2D(0.0, 0.0), 270.0,   Carrier.SideHight * 0.5)
    ...
    Carrier.g_pt6a =  Polar2D(Point2D(0.0, 0.0),  90.0,   (Carrier.SideHight * 0.5) + Carrier.PartGap)
  ...
  --[[
      ...
      if dialog:ShowDialog() then
        Carrier.Count             = dialog:GetIntegerField("Carrier.Count")
        Carrier.BottleDia         = dialog:GetDoubleField("Carrier.BottleDia")
        Carrier.BottleHight       = dialog:GetDoubleField("Carrier.BottleHight")
        ...
        if not IsNumberEven(Carrier.Count) then
  ...
  function DrawEnd(job, pt0, Layer)
    ...
    local pt3  = Polar2D(pt0,   90.0, Carrier.SideHight -(Carrier.MaterialBlockThickness + Carrier.DadoAllowance))
    local pt3a = Polar2D(pt3,  180.0, Carrier.MillingBitRad)
    local pt4  = Polar2D(pt0,   90.0, Carrier.SideHight)
    ...
    local pt12  = Polar2D(pt14, 90.0, Carrier.SideHight -  (Carrier.MaterialBlockThickness + Carrier.DadoAllowance))
    local pt12a = Polar2D(pt12,  0.0, Carrier.MillingBitRad)
    local pt11  = Polar2D(pt14, 90.0, Carrier.SideHight)
  ...
  function DrawHandle(job, pt0, Layer)
    local function Arc2Bulge (p1, p2, Rad)
      ...
      -- ======= Calulate Points
    local pt2 = Polar2D(pt0,   90.0, Carrier.HandleWidht)
    local pt4 = Polar2D(pt0,    0.0, Carrier.HandleLenght)
    local pt3 = Polar2D(pt4,   90.0, Carrier.HandleWidht)
    local ptCen = Polar2D(pt0,   0.0,  Carrier.HandleLenght * 0.5)
    local ptUp =  Polar2D(ptCen, 90.0, Carrier.HandleWidht - (Carrier.HandleTop + 0.35))
    ...
    line:AppendPoint(pt0)
    line:LineTo(pt2)
    line:ArcTo(pt3, -1.0 * Arc2Bulge (pt2, pt3, Carrier.HandleRad))
    line:LineTo(pt4)
    line:LineTo(pt0)
    layer:AddObject(CreateCadContour(line), true)
    ...
    line:LineTo(pt6)
    line:LineTo(pt8)
    line:ArcTo(pt7, -1.0 * Arc2Bulge (pt2, pt3, Carrier.HandleRad2))
    line:LineTo(pt5)
    layer:AddObject(CreateCadContour(line), true)
    job:Refresh2DView()
    return true
  ...
  -- ======= Calulate Points
    local pt2 = Polar2D(pt0,   90.0, Carrier.SideHight)
    local pt4 = Polar2D(pt0,    0.0, Carrier.SideLenght)
    local pt3 = Polar2D(pt4,   90.0, Carrier.SideHight)
    ...
    local pt6 = Polar2D(pt5,   90.0, Carrier.SideHight - (Carrier.MaterialBlockThickness  * 2) - Carrier.DadoAllowance)
    local pt7 = Polar2D(pt5,    0.0, Carrier.SideLenght - (Carrier.MaterialBlockThickness * 2) - Carrier.DadoAllowance)
    local pt8 = Polar2D(pt7,   90.0, Carrier.SideHight - (Carrier.MaterialBlockThickness  * 2) - Carrier.DadoAllowance)
  ...
  function REG_ReadRegistry(RegName)   -- Read Registry values
    ...
    Carrier.Count                  = RegistryRead:GetInt("Carrier.Count",                     6.00000)
    Carrier.Tabs                   = RegistryRead:GetBool("Carrier.Tabs",                      true)
    Carrier.BottleDia              = RegistryRead:GetDouble("Carrier.BottleDia",              2.50000)
    Carrier.BottleHight            = RegistryRead:GetDouble("Carrier.BottleHight",           11.0000)
  ...
  function CheckDialogValues()
    ...
    if Carrier.BottleHight == 0.0 then
      Carrier.DialogLoop = true
  ...
  function REG_UpdateRegistry(RegName) -- Write Registry values
    ...
    local RegValue = RegistryWrite:SetInt("Carrier.Count",                Carrier.Count)
          ...
          RegValue = RegistryWrite:SetDouble("Carrier.BottleHight",       Carrier.BottleHight)
