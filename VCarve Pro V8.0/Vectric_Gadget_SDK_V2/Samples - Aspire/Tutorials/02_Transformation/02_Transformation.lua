...
function main(script_path)
  ...
  local id = component_manager:AddRelief(relief, CombineMode.Add, "02 Transformation")
  
  -- Get the component back by its id
  local component = component_manager:FindObjectWithId(id)
  
  -- Transform it
  local rotation_centre = Point2D(0.5 * width, 0.5 * height)
  local rotation = RotationMatrix2D(rotation_centre, 45)
  local translation_vector = Vector2D(2.5, 2.5)
  local translation = TranslationMatrix2D(translation_vector)
  local xform = translation * rotation
