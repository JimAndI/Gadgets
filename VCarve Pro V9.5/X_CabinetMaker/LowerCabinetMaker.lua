...
g_cabinetHeight = 32.0000
g_cabinetDepth = 22.0000
g_pinOuter = 0.7000
g_pinInter = 0.1600
g_pt1 = Point2D(2,2)
...
function CutListfileWriter(datFile)
    ...
    local file = io.open(datFile.. "//LowerCutList" .. dY .. dM .. dD .. dMin ..".dat", "w")
    -- write line of the file
        ...
        file:write("Bottom Panels          - 1 Pcs of ".. g_materialThickness .." Plywood ".. g_boxDepth .." X ".. g_boxLength .."\n")
        file:write("Back Panel             - 1 Pcs of ".. g_materialThicknessBack .." Plywood 23.475 X 35.25\n")
        if g_faceFrameCenter == "Y" then
...
-- ==============================================================================
function LowerCabinetBack(lay)   -- Add Dato for the TopCab-CenterPanel
    local job = VectricJob()
    if not job.Exists then
        DisplayMessageBox("No job loaded")
        return false;
...
-- ==============================================================================
function LowerCabinetTandB(top, lay) -- Add Dato for the TopCab-CenterPanel
    local job = VectricJob()
    if not job.Exists then
        DisplayMessageBox("No job loaded")
        return false;
...
function main(script_path)
    ...
    lay = "LowerCab-LeftSide"     ;     LowerCabinetSide("L", lay)
    lay = "LowerCab-RightSide"    ;     LowerCabinetSide("R", lay)
    lay = "LowerCab-Top"          ;     LowerCabinetTandB("T", lay)
    lay = "LowerCab-Bottom"       ;     LowerCabinetTandB("B", lay)
    lay = "LowerCab-Back"         ;     LowerCabinetBack(lay) 
    ...
    --Loop here, draw each Shelf on it own Layer and the Lay name will number per count
        lay = "TopCab-Shelf".. g_shelfCountX        
        LowerCabinetShelf(lay, g_shelfCountX)
        g_shelfCountX = g_shelfCountX - 1
    end
    lay = "LowerCab-CenterPanel"  ;     LowerCenterPanel(lay)
