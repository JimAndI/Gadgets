...
function CutListfileWriter(datFile)
    ...
    -- write line of the file
        file:write("================================================================\n")
        file:write("===================== Upper Cabinet Cutlist ====================\n")
        file:write("----------------------------------------------------------------\n")
...
function UpperCenterPanel(lay)
    ...
    local line = Contour(0.0) ; line:AppendPoint(g_pt1) ; line:LineTo(pt2) ;  line:LineTo(pt3) ; line:LineTo(pt4) ; line:LineTo(g_pt1) 
    layer:AddObject(CreateCadContour(line), true) 
    local ptx = g_pt1
    local anx = 90
...
function LoadDialog(job, script_path)
    ...
    dialog:AddDoubleField("g_cabinetLenght",                    g_upperCabinetLenght)      
    dialog:AddDoubleField("g_cabinetHeight",                    g_upperCabinetHeight)
    dialog:AddDoubleField("g_cabinetDepth",                     g_upperCabinetDepth)      
...
-- Get the data from the Dialog and update my global variables
    ...
    g_upperCabinetLenght =      tonumber(dialog:GetDoubleField("g_cabinetLenght"))
    g_upperCabinetHeight =      tonumber(dialog:GetDoubleField("g_cabinetHeight"))
    g_shelfCount   =            tonumber(dialog:GetDoubleField("g_shelfCount"))
    g_upperCabinetDepth =       tonumber(dialog:GetDoubleField("g_cabinetDepth"))
...
function main(script_path)
    ...
    UpperCabinetSide("L", g_materialThickness .. "UpperCab-LeftSide")
    UpperCabinetSide("R", g_materialThickness .. "UpperCab-RightSide")
    UpperCabinetTandB("T", g_materialThickness .. "UpperCab-Top" )
    UpperCabinetTandB("B", g_materialThickness .. "UpperCab-Bottom")
    UpperCabinetBack(g_materialThicknessBack .. "UpperCab-Back") 
    ...
    --Loop here, draw each Shelf on it own Layer and the Lay name will number per count
        lay = g_materialThickness .. "UpperCab-Shelf".. g_shelfCountX        
        UpperCabinetShelf(lay, g_shelfCountX)
        g_shelfCountX = g_shelfCountX - 1
    end
    if g_faceFrameCenter2 ==  "Y" then
        UpperCenterPanel(g_materialThickness .. "UpperCab-CenterPanel")
    end
    UpperCabinetFaceFrame("UpperCab-FaceFrame")
--[[
--<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>

    LowerCabinetSide("L", g_materialThickness .. "LowerCab-LeftSide")
    LowerCabinetSide("R", g_materialThickness .. "LowerCab-RightSide")
    LowerCabinetTandB("T", g_materialThickness .. "LowerCab-Top" )
    LowerCabinetTandB("B", g_materialThickness .. "LowerCab-Bottom")
    LowerCabinetBack(g_materialThicknessBack .. "LowerCab-Back") 
    local g_shelfCountX = g_shelfCount
    while (g_shelfCountX > 0) do
    --Loop here, draw each Shelf on it own Layer and the Lay name will number per count
    ...
    if g_faceFrameCenter4 ==  "Y" then
        LowerCenterPanel(g_materialThickness .. "UpperCab-CenterPanel")
    end
    LowerCabinetFaceFrame("UpperCab-FaceFrame")
