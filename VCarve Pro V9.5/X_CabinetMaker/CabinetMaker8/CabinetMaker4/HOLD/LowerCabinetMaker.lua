...
-- ==============================================================================================
-- Lower Cabinet boxHight = (g_lowerCabinetHeight – (g_faceFrameTopWidth + g_faceFrameBottomWidth)) + ((g_faceFrameBottomOverlap + g_materialThickness) + ( g_materialThickness + g_faceFrameTopOverlap))
-- Lower Cabinet Box Hight = 27.8125 = (30.00 – (2.25 + 2.25)) + ((0.0625 + 0.75) + ( 0.75 + 0.75))
-- Lower Cabinet Back Hight = (boxHight – (g_materialThickness 2.0))
-- Lower Cabinet Back Hight = 27.0625 = (27.8125 – (0.375 * 2.0)) 
-- ==============================================================================================
-- Lower Cabinet boxLength = (g_lowerCabinetLenght – (g_faceframeSideGap * 2.0))
-- Lower Cabinet boxLenght = 35.50 = (36.00 – (0.25 * 2.0)) 
-- Lower Cabinet Back Width = (boxLength  – (g_datoDeep * 2.0))
-- Lower Cabinet Back Width = 34.75 = (35.50 – (0.375 * 2.0)) 
-- ==============================================================================
...
function CutListfileWriter(datFile)
    ...
    file:write("================================================================\n")
    file:write("===================== Lower Cabinet Cutlist ====================\n")
    file:write("----------------------------------------------------------------\n")
    file:write("Run Date = ".. A1 .."\n")
    file:write("================================================================\n")
    file:write("Lower Cabinet size\n")
    ...
    file:write("Face Frame Diagonal = ".. math.sqrt ((g_lowerCabinetHeight * g_lowerCabinetHeight)+ (g_lowerCabinetLenght * g_lowerCabinetLenght)) .."\n")
    file:write("----------------------------------------------------------------\n")
    -- Shelf info
...
function LowerCabinetFaceFrame(lay)   
    ...
    local layer = job.LayerManager:GetLayerWithName(lay)
    local pt2 = Polar2D(g_pt1, 0,   g_lowerCabinetHeight)
    local pt3 = Polar2D(pt2, 90,  g_lowerCabinetLenght)
    local pt4 = Polar2D(pt3, 180, g_lowerCabinetHeight)
    local line = Contour(0.0)
    ...
    g_Rails = g_lowerCabinetLenght - (g_faceFrameSideWidth * 2.0)
    g_centerStile = g_lowerCabinetHeight - (g_faceFrameBottomWidth + g_faceFrameTopWidth) 
...
-- ==============================================================================
function LowerCabinetBack(lay)   -- Add Dato for the LowerCab-CenterPanel
    local job = VectricJob()
    if not job.Exists then
        DisplayMessageBox("No job loaded")
        return false;
...
-- ==============================================================================
function LowerCabinetTandB(top, lay) -- Add Dato for the LowerCab-CenterPanel
    local job = VectricJob()
    if not job.Exists then
        DisplayMessageBox("No job loaded")
        return false;
...
function LoadDialog(job, script_path)
    local registry = Registry("LowerCabinetMaker")
	local html_path = "file:" .. script_path .. "\\LowerCabinetMaker.htm"
    ...
    dialog:AddDoubleField("g_cabinetLenght",                    g_lowerCabinetLenght)      
    dialog:AddDoubleField("g_cabinetHeight",                    g_lowerCabinetHeight)
    dialog:AddDoubleField("g_cabinetDepth",                     g_lowerCabinetDepth)      
...
-- Collect the Dialog values and update global variables
    ...
    g_lowerCabinetLenght =      tonumber(dialog:GetDoubleField("g_cabinetLenght"))
    ...
    g_lowerCabinetDepth =       tonumber(dialog:GetDoubleField("g_cabinetDepth"))
    ...
    g_faceFrameCenter2 =    string.upper(dialog:GetTextField("g_faceFrameCenter2"))
    g_faceFrameCenter =     string.upper(dialog:GetTextField("g_faceFrameCenter"))
...
function main(script_path)
    ...
    g_boxHight =  g_lowerCabinetHeight - (g_faceFrameTopWidth + g_faceFrameBottomWidth + g_faceFrameBottomOverlap + g_faceFrameTopOverlap + (g_materialThickness * 2.0))
    g_boxLength = g_lowerCabinetLenght - (g_faceFrameSideGap * 2.0) ;     g_backHight = g_boxHight - g_materialThickness ;     g_backWidth = g_boxLength - g_materialThickness ;     g_boxDepth  = g_lowerCabinetDepth - g_faceFrameThickness
    lay = g_materialThickness .. "LowerCab-LeftSide"     ;     LowerCabinetSide("L", lay)
    lay = g_materialThickness .. "LowerCab-RightSide"    ;     LowerCabinetSide("R", lay)
    lay = g_materialThickness .. "LowerCab-Top"          ;     LowerCabinetTandB("T", lay)
    lay = g_materialThickness .. "LowerCab-Bottom"       ;     LowerCabinetTandB("B", lay)
    lay = g_materialThicknessBack .. "LowerCab-Back"         ;     LowerCabinetBack(lay) 
    ...
    --Loop here, draw each Shelf on it own Layer and the Lay name will number per count
        lay = g_materialThickness .. "LowerCab-Shelf".. g_shelfCountX        
        LowerCabinetShelf(lay, g_shelfCountX)
        g_shelfCountX = g_shelfCountX - 1
    end
    
    if g_faceFrameCenter2 ==  "Y" then
        lay = g_materialThickness .. "LowerCab-CenterPanel"  
        LowerCenterPanel(lay)
    end 
    lay = "LowerCab-FaceFrame"    ;     LowerCabinetFaceFrame(lay)
