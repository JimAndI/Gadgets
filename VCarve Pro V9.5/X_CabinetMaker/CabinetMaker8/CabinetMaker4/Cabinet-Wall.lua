...
function XXX(ObjName)
    ...
    ObjName.StandardsPath = RegistryRead:GetString("StandardsPath",  ObjName.StandardsPath)
 
end
-- ==============================================================================
function WriteStandardCabnetList(ObjName)
    local RegistryRead = Registry(ObjName.RegistryName)
    ObjName.ProgramTitle = RegistryRead:GetString("ProgramTitle", ObjName.ProgramTitle)
    ObjName.ProgramName = RegistryRead:GetString("ProgramName",ObjName.ProgramName)
    ObjName.ProgramVersion = RegistryRead:GetString("ProgramVersion",ObjName.ProgramVersion)
...
-- ==============================================================================
function ReadStandardCabinetList(ObjName)
    local RegistryRead = Registry(ObjName.RegistryName)
    ObjName.ProgramTitle = RegistryRead:GetString("ProgramTitle", ObjName.ProgramTitle)
    ObjName.ProgramName = RegistryRead:GetString("ProgramName",ObjName.ProgramName)
    ObjName.ProgramVersion = RegistryRead:GetString("ProgramVersion",ObjName.ProgramVersion)
...
function Save2Registry(ObjName)
    ...
    RegistryWrite:SetString("StandardsPath", ObjName.StandardsPath)    
...
function CutListfileWriter(datFile)
    ...
    -- Get the file open for writing lines to the file
        ...
        if Shelf.Count >= 1 then
            file:write("Face Frame Shelf       - ".. Shelf.Count .." pcs of ".. FaceFrame.BottomRailWidth .." Hardwood ".. Shelf.FaceFrameThickness .." X ".. Shelf.PartLength .."     \n")
        end
        if Shelf.Count >= 1 then
...
-- bottom dato
    ...
    if Wall.AddShelf then
        ...
        while (rows > 0) do
            Holer (ptx, anx, spc, Wall.PinRadus, lay)
...
-- =================================================================================
function InquiryBox(Header, Quest, Defaltxt)
-- local x = InquiryBox("Cabinet Maker", "Enter your last name?", "Anderson")
...
function LoadDialog(job, script_path)
   -- local registry = Registry("CabinetMaker")
    ...
    dialog:AddLabelField("StandardsPath",                Global.StandardsPath)
    ...
    dialog:AddTextField("StandardsPath",                 Global.StandardsPath);
    dialog:AddDirectoryPicker("DirectoryPicker2",  "StandardsPath", true);   
...
-- Get the data from the Dialog and update my global variables
    ...
    Global.StandardsPath            =          dialog:GetTextField("StandardsPath")
	...
	Milling.Tool                    = tool    
--    Milling.ToolNameLabel           = dialog:GetTextField("ToolNameLabel")
...
function main(script_path)
    ...
    if Wall.AddFaceFrame then   
        ...
        g_boxDepth  = Wall.CabDepth - FaceFrame.BottomRailWidth
    else
    ...
    if StandardCabs.New then
        local xa = InquiryBox("Cabinet Maker", "Standard Cabinet Name", "Style-0001")
        if xa ~= "" then
            fileWriter(Global.StandardsPath .. "//" .. xa .. ".std")
        end
    end
