...
if not job then
   MessageBox("Please create a job before trying to use this gadget")
  return false
end
  ...
  -- if we use a ball nose then we should do another pass at the start height
  -- if options.tool.ToolType == BALL_NOSE then
  ...
  while p_selected ~= nil do
    ...
    if current_contour.IsOpen then
      ...
      if not inner_offset then
        MessageBox("Original offsetting with distance" .. signed_offset_dist .. " failed.\n Check for nearly closed vectors, loops and corssings\n")
        return true
       end
      ...
      -- add this contour in as well otherwise doesn't work
      offset_contours:AddHead(current_contour:Clone());
    end
